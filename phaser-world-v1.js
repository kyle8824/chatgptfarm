...
function currentModel(w=canonical){const d=(w?.dna||[]).find(x=>x?.mind?.brain_mode==='ai'&&x?.mind?.model);return d?.mind?.model||w?.meta?.mindModel||null}
...
class LivingWorld extends Phaser.Scene{
 ...
 renderResourceLandscape(){
  ...
  for(const o of canonical.worldModel?.objects||[]){if(o.state?.active===false||o.parentId||!o.position||!['stone_field','clay_bank','reed_marsh','berry_patch'].includes(o.type))continue;const p=worldToPx(o.position),R=seeded(`${o.id}:resource-v1`);
   ...
   if(o.type==='clay_bank'){
    const ratio=clamp((res.clay||0)/10,0,1);state.clayRichness=+ratio.toFixed(2);
    const river=this.riverCurve?.getSpacedPoints(220)||[];let center=null,t={x:1,y:0},n={x:0,y:1},bestD=Infinity;
    if(river.length>2){let bi=0;for(let i=0;i<river.length;i++){const d=Math.hypot(p.x-river[i].x,p.y-river[i].y);if(d<bestD){bestD=d;bi=i}}center=river[bi];const a=river[Math.max(0,bi-3)],b=river[Math.min(river.length-1,bi+3)],dx=b.x-a.x,dy=b.y-a.y,m=Math.hypot(dx,dy)||1;t={x:dx/m,y:dy/m};n={x:-t.y,y:t.x};if((p.x-center.x)*n.x+(p.y-center.y)*n.y<0)n={x:-n.x,y:-n.y}}
    const bankOffset=41,shore=center?{x:center.x+n.x*bankOffset,y:center.y+n.y*bankOffset}:p,P=(along,land)=>({x:shore.x+t.x*along+n.x*land,y:shore.y+t.y*along+n.y*land}),path=(g,pts)=>{g.beginPath();g.moveTo(pts[0].x,pts[0].y);for(let i=1;i<pts.length;i++)g.lineTo(pts[i].x,pts[i].y);g.closePath();g.fillPath()},half=46+24*ratio;
    this.clayBankProjection={anchored:!!center,sourceDistancePx:Number.isFinite(bestD)?+bestD.toFixed(1):null,bankOffsetPx:bankOffset,worldAnchor:pxToWorld(shore),ratio:+ratio.toFixed(2)};
    const wash=keep(this.add.graphics().setDepth(14));wash.fillStyle(0x5b4b3d,.28);path(wash,[P(-half-10,-19),P(-half*.48,-25),P(5,-28),P(half*.5,-24),P(half+8,-14),P(half*.7,-4),P(0,-8),P(-half*.72,-6)]);
    const face=keep(this.add.graphics().setDepth(16));face.fillStyle(0x744a38,.96);path(face,[P(-half,8),P(-half*.78,-3),P(-half*.38,-8),P(-3,-5),P(half*.38,-9),P(half*.82,-3),P(half,7),P(half*.72,18),P(half*.26,22),P(-half*.22,20),P(-half*.72,16)]);
    face.fillStyle(0x9b6247,.76+.16*ratio);path(face,[P(-half*.92,6),P(-half*.62,-1),P(-half*.2,-4),P(half*.18,-2),P(half*.62,-6),P(half*.9,1),P(half*.7,8),P(half*.18,12),P(-half*.34,10),P(-half*.75,12)]);
    const lip=keep(this.add.graphics().setDepth(17));lip.fillStyle(0x6f7658,.96);path(lip,[P(-half-8,20),P(-half*.72,14),P(-half*.3,12),P(0,15),P(half*.34,11),P(half*.76,14),P(half+8,20),P(half*.72,27),P(half*.18,25),P(-half*.3,28),P(-half*.78,25)]);lip.lineStyle(2,0xb77d5d,.62);const rim=[P(-half*.88,13),P(-half*.45,8),P(-5,10),P(half*.42,7),P(half*.86,11)];lip.strokePoints(rim,false,false);
    const detail=keep(this.add.graphics().setDepth(18)),scars=3+Math.round((1-ratio)*6);detail.lineStyle(1.4,0x55392f,.5);for(let i=0;i<scars;i++){const u=-half*.68+R()*half*1.36,v=1+R()*14,len=5+R()*10,q=P(u,v),q2=P(u+(R()-.5)*5,v-len);detail.beginPath();detail.moveTo(q.x,q.y);detail.lineTo(q2.x,q2.y);if(R()<.55){const q3=P(u+(R()-.5)*8,v-len-4);detail.lineTo(q3.x,q3.y)}detail.strokePath()}
    const clods=Math.max(1,Math.round(6*ratio));for(let i=0;i<clods;i++){const u=(R()-.5)*half*1.55,v=-7-R()*15,q=P(u,v),w=7+R()*11,h=4+R()*6;detail.fillStyle(i%3===0?0xb87c59:0x8f5b44,.68+.18*ratio);detail.fillEllipse(q.x,q.y,w,h)}
    continue}
 ...
 updateHud(){
  ...
  const aiModel=currentModel(canonical);s.innerHTML=`<b>● OBJECT WORLD · ${v}</b><span>Day ${canonical.day} · ${String(canonical.hour).padStart(2,'0')}:00 · ${Math.round(canonical.temperature)}°F · ${canonical.weather}</span><small id="phAiKey" class="phAiKey">✦ = ${escapeHtml(modelLabel(aiModel))}</small>`;
...
if(canonical.runtime){
 ...
 if(paused||late||this.connectionFailed)$('#phPulseText').textContent=paused?'World paused.':'Waiting for the next saved simulation update.';
 return;
}
const age=updateAge(),stale=!Number.isFinite(age)||age>playbackMs()*2;const status=$('#phWorldStatus');status.dataset.freshness=this.connectionFailed?'offline':stale?'stale':'current';status.querySelector('b').textContent=this.connectionFailed?'CONNECTION INTERRUPTED':stale?'UPDATES DELAYED':'WORLD CONNECTED';const minutes=Number.isFinite(age)?Math.floor(age/60000):null;status.title=minutes==null?'Update time unavailable':`Last saved ${minutes} minutes ago`;let note=$('#phFreshness');if(!note){note=document.createElement('small');note.id='phFreshness';status.append(note)}note.textContent=minutes==null?'Update time unknown':minutes<1?'Saved just now':`Saved ${minutes}m ago`;if(!Number.isFinite(age)||age>=playbackMs())$('#phFocus small').textContent='LAST RECORDED ACTIVITY';if(stale||this.connectionFailed){$('#phPulseText').textContent='Showing the last saved world. Waiting for a new update.'}if(this.decisionCue)this.decisionCue.setVisible(!stale);}
 ...
 showInspector(selection){
    ...
    if(selection.kind==='agent'){const m=currentModel(w);autonomy.textContent=m?`✦ AI · ${modelLabel(m)}`:'✦ AI CONTROLLED'}
