...
if(motion){
 const now=Date.now()+(this.serverOffset||0),t=clamp((now-motion.start)/(motion.end-motion.start),0,1),from=worldToPx(motion.from),to=worldToPx(motion.to),off=e.getData('runtimeOffset')||{x:0,y:0};
 ...
 const active=now<canonical.runtime.end&&!resting;
