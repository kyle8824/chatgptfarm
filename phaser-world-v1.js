...
class LivingWorld extends Phaser.Scene{
 constructor(){super('LivingWorld');this.entities=new Map();this.treeSprites=[];this.ambient=[];this.traceVisuals=[];this.signVisuals=[];this.routeVisuals=[];this.resourceVisuals=[];this.weatherMemoryVisuals=[];this.decisionCue=null;this.lastDecisionCueId=null;this.lightOverlay=null;this.nightLights=[];this.grid=null;this.drag=null;this.assetOk={};this.lastRenderedTick=null}
 preload(){
 ...
 refreshDynamicWorld(){this.renderTravelWear();this.renderResourceLandscape();this.renderCanonicalTraces();this.renderCanonicalSigns();this.renderEntities(false);this.updateDecisionCue(false);this.renderLightCycle(true);this.renderWeatherMemory(true);this.renderWeather(true);this.refreshAmbient()}
