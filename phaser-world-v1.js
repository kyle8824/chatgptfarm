 ...
 renderResourceLandscape(){
  ...
  for(const o of canonical.worldModel?.objects||[]){if(o.state?.active===false||o.parentId||!o.position||!['stone_field','clay_bank','reed_marsh','berry_patch'].includes(o.type))continue;const p=worldToPx(o.position),R=seeded(`${o.id}:resource-v1`);
   ...
   if(o.type==='clay_bank'){
    ...
    const ground=keep(this.add.graphics().setDepth(16));
    ground.fillStyle(0x4e4637,.16);ground.beginPath();ground.moveTo(p.x-70,p.y+12);ground.lineTo(p.x-45,p.y+4);ground.lineTo(p.x-12,p.y+1);ground.lineTo(p.x+28,p.y+4);ground.lineTo(p.x+64,p.y+15);ground.lineTo(p.x+43,p.y+24);ground.lineTo(p.x+4,p.y+27);ground.lineTo(p.x-38,p.y+23);ground.closePath();ground.fillPath();
    const face=keep(this.add.graphics().setDepth(17));
    face.fillStyle(0x684738,.88);face.beginPath();face.moveTo(p.x-50,p.y-7);face.lineTo(p.x-28,p.y-12);face.lineTo(p.x+4,p.y-10);face.lineTo(p.x+38,p.y-6);face.lineTo(p.x+51,p.y+2);face.lineTo(p.x+35,p.y+14);face.lineTo(p.x+4,p.y+18);face.lineTo(p.x-32,p.y+13);face.lineTo(p.x-52,p.y+4);face.closePath();face.fillPath();
    face.fillStyle(0x523a31,.46);face.beginPath();face.moveTo(p.x-42,p.y+5);face.lineTo(p.x-13,p.y+9);face.lineTo(p.x+13,p.y+7);face.lineTo(p.x+39,p.y+2);face.lineTo(p.x+31,p.y+12);face.lineTo(p.x+3,p.y+17);face.lineTo(p.x-28,p.y+13);face.closePath();face.fillPath();
    const shelf=keep(this.add.graphics().setDepth(18)),half=28+22*ratio;
    shelf.fillStyle(0xaa7254,.78+.14*ratio);shelf.beginPath();shelf.moveTo(p.x-half,p.y-8);shelf.lineTo(p.x-half*.58,p.y-17);shelf.lineTo(p.x-5,p.y-15);shelf.lineTo(p.x+half*.48,p.y-18);shelf.lineTo(p.x+half,p.y-9);shelf.lineTo(p.x+half*.72,p.y-1);shelf.lineTo(p.x+8,p.y+3);shelf.lineTo(p.x-half*.68,p.y);shelf.closePath();shelf.fillPath();
    shelf.lineStyle(2,0xd1a17d,.28+.26*ratio);shelf.beginPath();shelf.moveTo(p.x-half,p.y-8);shelf.lineTo(p.x-half*.58,p.y-17);shelf.lineTo(p.x-5,p.y-15);shelf.lineTo(p.x+half*.48,p.y-18);shelf.lineTo(p.x+half,p.y-9);shelf.strokePath();
    const clods=Math.max(1,Math.round(5*ratio)),detail=keep(this.add.graphics().setDepth(19));
    for(let i=0;i<clods;i++){const side=i%2?-1:1,x=p.x+side*(25+R()*26),y=p.y+5+R()*12,w=9+R()*10,h=5+R()*6;detail.fillStyle(i%3===0?0xb87c59:0x956147,.72+.16*ratio);detail.fillEllipse(x,y,w,h)}
    const scars=2+Math.round((1-ratio)*7);detail.lineStyle(1.5,0x5a4035,.48);for(let i=0;i<scars;i++){const x=p.x-32+R()*64,y=p.y-12+R()*20,len=5+R()*9;detail.beginPath();detail.moveTo(x,y);detail.lineTo(x+len,y+3+R()*5);if(R()<.55)detail.lineTo(x+len-3,y+7+R()*4);detail.strokePath()}
    continue}
