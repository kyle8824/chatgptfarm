...
function agentPresentationOffset(a,w=canonical){const mine=canonicalAgentPoint(a,w),near=(w?.agents||[]).some(x=>x.id!==a.id&&dist(mine,canonicalAgentPoint(x,w))<.35);let x=near?(a.id==='agent-mara'?-14:14):0,y=near?(a.id==='agent-mara'?2:-2):0;const camp=(w?.worldModel?.objects||[]).find(o=>o.type==='camp_area'&&o.state?.active!==false);if(a?.position==='camp'&&camp?.position&&dist(mine,camp.position)<3.5){x+=a.id==='agent-mara'?-52:44;y+=20}return{x,y}}
function currentPhase(a){if(a?.task)return {phase:a.task.phase,label:a.task.phase==='travel'?'TRAVELING':'ACTING'};const active=a?.activeAction;if(!active)return{phase:'idle',label:'IDLE'};const p=(active.phases||[]).find(x=>x.id==='travel'&&active.moving)||(active.phases||[]).find(x=>x.id==='interact')||(active.phases||[]).at(-1);return{phase:p?.id||'acting',label:String(p?.label||'ACTING').toUpperCase()}}
...
function cleanMemoryText(v){let text=String(v??'').trim(),sharedBy=null,hops=0;for(let i=0;i<8;i++){const m=text.match(/^([^:]{1,40}) told me:\s*/i);if(!m)break;if(!sharedBy)sharedBy=m[1].trim();text=text.slice(m[0].length).trim();hops++}return{text:text||'Recorded experience',sharedBy,hops}}
function currentModel(w=canonical){return w?.agents?.find(a=>a.mind?.brainMode==='ai'&&a.mind?.model)?.mind.model||null}
function modelLabel(v){const s=String(v||'').trim();if(!s)return'AI model';return s.replace(/^gpt-/i,'GPT-').replace(/-([a-z])/g,(_m,c)=>` ${c.toUpperCase()}`)}
...
class LivingWorld extends Phaser.Scene{
 ...
 drawCanonicalObjects(){for(const o of canonical.worldModel?.objects||[]){if(o.state?.active===false||o.parentId||o.type==='creek_segment'||o.type==='frontier')continue;if(['stone_field','clay_bank','reed_marsh','berry_patch'].includes(o.type))continue;const p=worldToPx(o.position||{x:50,y:50});if(o.type==='camp_area'){continue}if(o.type==='fallen_tree'){const len=(o.geometry?.lengthM||4.8)/(canonical.worldModel.bounds?.metersPerUnit||2)*TILE,g=this.add.rectangle(p.x,p.y,len,16,0x7c5738).setDepth(1000+p.y).setRotation(((o.geometry?.orientationDeg||0)-90)*Math.PI/180);this.add.rectangle(p.x-2,p.y-3,len*.82,3,0xa2744b,.55).setDepth(g.depth+1).setRotation(g.rotation);continue}if(o.type==='stone_field'){const R=seeded(o.id);for(let i=0;i<13;i++){const a=R()*Math.PI*2,rr=Math.sqrt(R())*58,x=p.x+Math.cos(a)*rr,y=p.y+Math.sin(a)*rr*.55;this.add.ellipse(x,y,8+R()*12,5+R()*7,0x8c9690,.95).setDepth(1000+y)}continue}if(o.type==='clay_bank'){const top=this.add.ellipse(p.x,p.y-2,118,45,0xa96f4d,.82).setDepth(18),face=this.add.ellipse(p.x+6,p.y+13,105,28,0x744a38,.82).setDepth(17);top.setStrokeStyle(2,0xc9956c,.4);face.setScale(1,.45);continue}if(o.type==='reed_marsh'){const R=seeded(o.id);this.add.ellipse(p.x,p.y+4,190,108,0x587569,.22).setDepth(11);for(let i=0;i<28;i++){const a=R()*Math.PI*2,rr=Math.sqrt(R())*82,x=p.x+Math.cos(a)*rr,y=p.y+Math.sin(a)*rr*.55;const r=this.add.image(x,y,'reeds').setOrigin(.5,1).setScale(.3+.18*R()).setDepth(1000+y);this.tweens.add({targets:r,angle:(R()-.5)*3,duration:1300+R()*1200,yoyo:true,repeat:-1,ease:'Sine.InOut'})}continue}if(o.type==='berry_patch'){const R=seeded(o.id);for(let i=0;i<9;i++){const a=R()*Math.PI*2,rr=Math.sqrt(R())*58,x=p.x+Math.cos(a)*rr,y=p.y+Math.sin(a)*rr*.52;this.add.image(x,y,'berry-bush').setOrigin(.5,1).setScale(.42+.08*R()).setDepth(1000+y)}}}
  this.campVisualMode='canonical-lean-to-v4-layered';const shelter=(canonical.worldModel?.objects||[]).find(o=>o.type==='branch_shelter'&&o.state?.active);if(shelter){const p=worldToPx(shelter.position),d=1000+p.y,R=seeded('branch-shelter-lean-to-v4');this.add.ellipse(p.x+3,p.y+17,134,38,0x17251b,.23).setDepth(d-5);
   const rear=this.add.graphics().setDepth(d-4);rear.fillStyle(0x25261f,.82);rear.fillRoundedRect(p.x-34,p.y-24,72,36,7);rear.fillStyle(0x1f211d,.72);rear.fillEllipse(p.x+9,p.y+8,48,13);
   const front=this.add.graphics().setDepth(d+72);front.fillStyle(0x5f5039,.98);front.beginPath();front.moveTo(p.x-52,p.y-34);front.lineTo(p.x+38,p.y-30);front.lineTo(p.x+55,p.y-7);front.lineTo(p.x-37,p.y-10);front.closePath();front.fillPath();front.fillStyle(0x43513a,.62);for(let k=0;k<9;k++){const x=p.x-39+k*10.5+R()*5,y=p.y-24+(k%3)*5+R()*3;front.fillEllipse(x,y,20+R()*15,7+R()*6)}front.lineStyle(5,0x4d3828,1);front.lineBetween(p.x-38,p.y+14,p.x-38,p.y-34);front.lineBetween(p.x+42,p.y+14,p.x+40,p.y-30);front.lineBetween(p.x-52,p.y-34,p.x+40,p.y-30);front.lineStyle(2.2,0x8e6b49,.8);for(let k=0;k<7;k++){const t=k/6,x1=p.x-46+t*84,y1=p.y-31+t*2,x2=x1+17,y2=p.y-9+t*1.3;front.lineBetween(x1,y1,x2,y2)}
   this.shelterLayering={mode:'layered-occlusion-v1',rearDepth:d-4,frontDepth:d+72,roofTop:p.y-34,roofFront:p.y-7,interiorBaseline:p.y+20};
  }const fire=(canonical.worldModel?.objects||[]).find(o=>o.type==='camp_fire'&&o.state?.active);if(fire){const p=worldToPx(fire.position),d=1000+p.y+2,glow=this.add.circle(p.x,p.y+2,29,0xf4a64f,.09).setDepth(d-3);this.tweens.add({targets:glow,scale:1.2,alpha:.035,duration:760,yoyo:true,repeat:-1});const logs=this.add.graphics().setDepth(d-1);logs.lineStyle(6,0x5e3b24,1);logs.lineBetween(p.x-14,p.y+8,p.x+13,p.y-3);logs.lineBetween(p.x-13,p.y-3,p.x+14,p.y+8);const flame=this.add.triangle(p.x,p.y-7,0,25,10,0,20,25,0xe97832,.98).setDepth(d);const inner=this.add.ellipse(p.x,p.y,8,16,0xffc65c,.95).setDepth(d+1);this.tweens.add({targets:[flame,inner],scaleX:1.08,scaleY:.88,y:'-=2',duration:430,yoyo:true,repeat:-1,ease:'Sine.InOut'});for(let i=0;i<3;i++){const smoke=this.add.circle(p.x+(i-1)*3,p.y-21-i*5,4-i*.7,0xd7ded7,.14).setDepth(d+1);this.tweens.add({targets:smoke,y:smoke.y-36,x:smoke.x+(i-1)*8,alpha:0,scale:1.8,duration:2100+i*420,repeat:-1,delay:i*390})}}
 }
 renderResourceLandscape(){
  ...
  for(const o of canonical.worldModel?.objects||[]){if(o.state?.active===false||o.parentId||!o.position||!['stone_field','clay_bank','reed_marsh','berry_patch'].includes(o.type))continue;const p=worldToPx(o.position),R=seeded(`${o.id}:resource-v1`);
   ...
   if(o.type==='berry_patch'){const ratio=clamp((canonical.regions?.sites?.[o.id]?.quantity??res.berries??0)/(canonical.regions?.sites?.[o.id]?.capacity||20),0,1),fruiting=Math.round(9*ratio);if(o.id==='OBJ-BERRIES-001')state.berryFruitingBushes=fruiting;else{state.siteFruitingBushes??={};state.siteFruitingBushes[o.id]=fruiting;}for(let i=0;i<9;i++){const a=R()*Math.PI*2,rr=Math.sqrt(R())*58,x=p.x+Math.cos(a)*rr,y=p.y+Math.sin(a)*rr*.52,key=i<fruiting?'berry-bush':'berry-bush-empty';keep(this.add.image(x,y,key).setOrigin(.5,1).setScale(.42+.08*R()).setAlpha(.92).setDepth(1000+y))}}
  }
 ...
 updateDecisionCue(){if(this.decisionCue){this.decisionCue.destroy();this.decisionCue=null}this.lastDecisionCueId=null}
 upsertAgent(a,initial){let e=this.entities.get(a.id);if(!e){const key=a.id==='agent-mara'?'person-mara':'person-ivo',target=worldToPx(canonicalAgentPoint(a));e=this.add.image(target.x,target.y,key).setOrigin(.5,1);const h=52;e.setDisplaySize(h*e.width/e.height,h).setDepth(1000+target.y+3);e.setData('kind','agent');e.setData('id',a.id);e.setData('artMode','natural-procedural');const name=this.add.text(target.x,target.y-59,a.name,{fontFamily:'Arial',fontSize:'10px',fontStyle:'bold',color:'#edf4e9',stroke:'#172018',strokeThickness:3}).setOrigin(.5).setDepth(9000).setAlpha(.9);const ai=this.add.text(target.x,target.y-76,'✦',{fontFamily:'Arial',fontSize:'13px',fontStyle:'bold',color:'#e9ffe1',stroke:'#102015',strokeThickness:3}).setOrigin(.5).setDepth(9001).setAlpha(.94);e.setData('label',name);e.setData('aiBadge',ai);this.entities.set(a.id,e)}e.getData('aiBadge')?.setVisible(a.mind?.brainMode==='ai');const fallback=canonicalAgentPoint(a),to=worldToPx(a.activeAction?.to||fallback),from=worldToPx(a.activeAction?.from||fallback),off=agentPresentationOffset(a);for(const p of [from,to]){p.x+=off.x;p.y+=off.y}if(a.runtimeMotion){let motion=a.runtimeMotion;if(canonical.runtime?.execution==='elapsed-only'){const last=e.getData('lastElapsedPoint')||fallback;const now=Date.now()+(this.serverOffset||0);motion={from:last,to:fallback,path:[last,fallback],start:now,end:now+800};e.setData('lastElapsedPoint',{...fallback});}e.setData('runtimeMotion',motion);e.setData('runtimeOffset',off);e.setData('activity',a.currentAction);this.tweens.killTweensOf(e)}else this.applyRecordedMotion(e,from,to,'walking');}
 ...
 updateHud(){
  ...
  const aiModel=currentModel(canonical);s.innerHTML=`<b>● OBJECT WORLD · ${v}</b><span>Day ${canonical.day} · ${String(canonical.hour).padStart(2,'0')}:${String(canonical.minute||0).padStart(2,'0')} · ${Math.round(canonical.temperature)}°F · ${canonical.weather}</span><small id="phAiKey" class="phAiKey">${aiModel?`✦ = ${escapeHtml(modelLabel(aiModel))}`:'RULE-BASED FALLBACK'}</small>`;
  const agents=(canonical.agents||[]).slice(0,2);
  $('#phFocus').innerHTML=`<small>AGENTS NOW</small><div class="phAgentsNow">${agents.map(a=>{const ph=currentPhase(a),place=friendlyLabel(a?.position||'world'),activity=a?.activeAction?.label||a?.currentAction||a?.mind?.currentGoal||'Observing the basin.';return `<div class="phAgentNow" data-agent-now="${escapeHtml(a.id)}"><b>${escapeHtml(a.name)}</b><span>${escapeHtml(activity)}</span><em>${escapeHtml(ph.label)} · ${escapeHtml(place)}</em></div>`}).join('')}</div>`;
 ...
 closeInspector(){const p=$('#phInspector');if(p)p.hidden=true;this.selected=null}
 wireUI(){document.querySelectorAll('[data-camera]').forEach(b=>b.addEventListener('click',()=>{this.closeInspector();cameraMode=b.dataset.camera;this.applyCamera(true);this.syncButtons()}));$('#phZoomIn')?.addEventListener('click',()=>this.setZoom(this.cameras.main.zoom+.12));$('#phZoomOut')?.addEventListener('click',()=>this.setZoom(this.cameras.main.zoom-.12));$('#phHome')?.addEventListener('click',()=>{this.closeInspector();cameraMode='auto';this.applyCamera(true);this.syncButtons()})}
...
if(canonical.runtime){
 const status=$('#phWorldStatus'),paused=canonical.runtime.status==='paused',late=Date.now()+(this.serverOffset||0)>canonical.runtime.end+2000;
 status.dataset.freshness=this.connectionFailed?'offline':late&&!paused?'stale':'current';
 status.querySelector('b').textContent=this.connectionFailed?'CONNECTION INTERRUPTED':paused?'WORLD PAUSED':late?'LIVE UPDATE DELAYED':'WORLD LIVE';
 status.title=`Decision source: ${canonical.runtime.decisionSource}`;
 if(paused||late||this.connectionFailed)$('#phPulseText').textContent=paused?'World paused.':this.connectionFailed?'Connection interrupted. Showing the last received activity.':'Live update delayed. Showing the last received activity.';
 return;
}
const age=updateAge(),stale=!Number.isFinite(age)||age>playbackMs()*2;const status=$('#phWorldStatus');status.dataset.freshness=this.connectionFailed?'offline':stale?'stale':'current';status.querySelector('b').textContent=this.connectionFailed?'CONNECTION INTERRUPTED':stale?'UPDATES DELAYED':'WORLD CONNECTED';const minutes=Number.isFinite(age)?Math.floor(age/60000):null;status.title=minutes==null?'Update time unavailable':`Last saved ${minutes} minutes ago`;let note=$('#phFreshness');if(!note){note=document.createElement('small');note.id='phFreshness';status.append(note)}note.textContent=minutes==null?'Update time unknown':minutes<1?'Saved just now':`Saved ${minutes}m ago`;if(!Number.isFinite(age)||age>=playbackMs())$('#phFocus small').textContent='LAST RECORDED ACTIVITY';if(stale||this.connectionFailed){$('#phPulseText').textContent='Showing the last saved world. Waiting for a new update.'}if(this.decisionCue)this.decisionCue.setVisible(!stale);}
 ensureInspector(){
    ...
    $('#phInspectClose').onclick=()=>this.closeInspector();
    document.addEventListener('keydown',e=>{if(e.key==='Escape')this.closeInspector()});
    document.addEventListener('pointerdown',e=>{if(!p.hidden&&!p.contains(e.target))this.closeInspector()},{capture:true});
  }
 showInspector(selection){
    ...
    if(selection.kind==='agent'){const m=d.mind;autonomy.textContent=m?.brainMode==='ai'?`✦ AI PLAN · ${modelLabel(m.model)}`:`RULES · ${friendlyLabel(m?.fallbackReason||'fallback')}`}
    ...
    if(selection.kind==='agent'){
      ...
      const needs=[
        ...
        ['Hunger',Number.isFinite(d.needs?.hunger)?100-d.needs.hunger:undefined],
        ['Energy',d.needs?.energy],
        ['Warmth',d.needs?.warmth]
      ].filter(([,v])=>Number.isFinite(Number(v)));
      const meter=(label,value)=>{
        const v=pct(value),health=label==='Hunger'?100-v:v,tone=health<35?'low':health<60?'watch':'good';
      ...
      const memories=(d.memories||[]).slice(0,5)
        .map(m=>{const clean=cleanMemoryText(m.text||m.detail||'Recorded experience'),kind=clean.sharedBy?`SHARED BY ${String(clean.sharedBy).toUpperCase()}`:'MEMORY';return `<article class="phMemory"><span>${escapeHtml(kind)}</span><p>${escapeHtml(clean.text)}</p><small>${escapeHtml((m.tags||[]).filter(x=>x!=='social').slice(0,3).join(' · ')||(clean.sharedBy?'shared knowledge':'personal experience'))}</small></article>`})
      ...
      const lowest=needs.slice().sort((a,b)=>(a[0]==='Hunger'?100-a[1]:Number(a[1]))-(b[0]==='Hunger'?100-b[1]:Number(b[1])))[0];
    ...
    if(selection.kind==='wildlife'){
      ...
      const material=typeof d.material==='string'?d.material.trim():(d.material&&Object.keys(d.material).length?JSON.stringify(d.material):'');
...
if(motion){
 ...
 let point={x:lerp(from.x,to.x,t),y:lerp(from.y,to.y,t)};
 if(motion.path?.length>1){const points=motion.path.map(worldToPx),lengths=points.slice(1).map((p,i)=>dist(points[i],p));let left=lengths.reduce((a,b)=>a+b,0)*t;for(let i=0;i<lengths.length;i++){if(left<=lengths[i]||i===lengths.length-1){const f=lengths[i]?clamp(left/lengths[i],0,1):1;point={x:lerp(points[i].x,points[i+1].x,f),y:lerp(points[i].y,points[i+1].y,f)};break;}left-=lengths[i];}}
 e.setPosition(point.x+off.x,point.y+off.y);
