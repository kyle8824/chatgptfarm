...
class LivingWorld extends Phaser.Scene{
 constructor(){super('LivingWorld');this.entities=new Map();this.treeSprites=[];this.ambient=[];this.traceVisuals=[];this.signVisuals=[];this.routeVisuals=[];this.decisionCue=null;this.lastDecisionCueId=null;this.lightOverlay=null;this.nightLights=[];this.grid=null;this.drag=null;this.assetOk={};this.lastRenderedTick=null}
 preload(){
 ...
 buildWorld(){
  ...
  this.drawOrganicSurface();this.renderTravelWear();this.drawBiomeUnderlays();this.drawMeadowMosaic();this.drawCreekDetails();this.spawnGroundDetail();this.spawnForest();this.drawCanonicalObjects();this.renderCanonicalTraces();this.renderCanonicalSigns();this.renderEntities(true);this.updateDecisionCue(true);this.spawnAmbientLife();this.renderLightCycle();this.renderWeather();
 }
 drawOrganicSurface(){
 ...
 refreshDynamicWorld(){this.renderTravelWear();this.renderCanonicalTraces();this.renderCanonicalSigns();this.renderEntities(false);this.updateDecisionCue(false);this.renderLightCycle(true);this.renderWeather(true);this.refreshAmbient()}
