...
export class RealtimeController{
 ...
 async makeDecision(id,input,choices){
  const a=this.record.world.agents.find(a=>a.id===id),route=providerFor(this.record.world,a,this.env,this.ai),payload={messages:[{role:'system',content:'You are a person surviving in a physical valley. Choose one currently available action. Prioritize urgent needs, then consider individual interests and recent satisfaction. Repeating a satisfied activity is less rewarding. Rest restores energy; shelter alone does not raise warmth. Learn from memories. Return JSON with actionId from choices, goal (short), intent (one sentence), decisionSummary (one sentence grounded in supplied facts). Never invent completed events or resources.'},{role:'user',content:input}],response_format:{type:'json_schema',json_schema:{type:'object',properties:{actionId:{type:'string',enum:choices.map(c=>c.id)},goal:{type:'string'},intent:{type:'string'},decisionSummary:{type:'string'}},required:['actionId','goal','intent','decisionSummary']}},max_tokens:180,temperature:.65};
