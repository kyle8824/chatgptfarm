...
export class RealtimeController{
 ...
 async requestDecisions(){if(!this.ai||this.persistenceRetryAt||this.now()-this.lastAiCheck<10000||!this.record||this.now()-this.record.lastWallTime>3000)return;this.lastAiCheck=this.now();void this.requestDesigns();
  for(const a of this.record.world.agents){if(!isAdult(this.record.world,a))continue;const pending=this.proposals.get(a.id);if(pending?.expires<=this.now()){this.proposals.delete(a.id);if(this.record.pendingDecisions)delete this.record.pendingDecisions[a.id];}
   ...
   const context=retrieveDecisionContext(this.record.world,a);const choices=liveCandidates(this.record.world,a,context.candidates).slice(0,8).map(x=>({id:x.id,label:x.label,reason:x.job?.reason||x.explanation||null}));
   const input=JSON.stringify({person:a.name,interests:freeTimeContext(this.record.world,a),needs:context.perception.needs,weather:context.perception.weather,temperature:context.perception.temperature,location:a.position,inventory:a.inventory,relationships:context.perception.knownPeople,memories:context.memories.slice(0,5).map(m=>m.text.slice(0,170)),recentActions:context.agent.recentActions.slice(-4),choices}).slice(0,6000);
 ...
 async requestDesigns(){
  const w=this.record.world;ensureSettlement(w);if(w.settlement.projects.filter(p=>p.status!=='complete').length>=2)return;
 ...
 async makeDesign(id,context){
  ...
  let raw;try{
   const repair=this.record.designRepair?.[id],fresh=(repair?.repeats||0)>=2;
   const result=await this.ai.run(MODEL,{messages:[{role:'system',content:DESIGN_SYSTEM},{role:'user',content:JSON.stringify({...context,previousProgram:fresh?null:this.record.designDrafts?.[id]||null,previousRejection:this.record.designFailures?.[id]||null,retryStrategy:fresh?'fresh_design':'revise',failedAttempts:repair?.repeats||0})}],response_format:{type:'json_object'},max_tokens:2600,temperature:.65});
   ...
   await this.serial(async()=>{const r=this.record,w=r.world,a=w.agents.find(a=>a.id===id),maintenance=raw?.repairProjectId?requestMaintenance(w,a,raw.repairProjectId,raw.rationale):null,design=maintenance?null:validateBlueprint(w,a,raw,context.sites?.find(s=>s.id===raw.siteId)),project=adoptBlueprint(w,a,design,MODEL);r.ai.succeeded++;(r.designFailures??={})[id]=null;if(r.designRepair)delete r.designRepair[id];if(r.designDrafts)delete r.designDrafts[id];
    w.settlement.designs.push({at:this.now(),agentId:id,status:maintenance?'maintenance_requested':project?'accepted':'deferred',projectId:project?.id||maintenance?.id||null,model:MODEL,reason:project?.rationale||String(raw.rationale||'No useful new structure proposed.').slice(0,350)});w.settlement.designs=w.settlement.designs.slice(-30);await this.save();});
