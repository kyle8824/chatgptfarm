...
export class RealtimeController{
 ...
 async makeDesign(id,context){
  ...
  try{
   const result=await this.ai.run(MODEL,{messages:[{role:'system',content:DESIGN_SYSTEM},{role:'user',content:JSON.stringify({...context,previousRejection:this.record.designFailures?.[id]||null})}],response_format:{type:'json_object'},max_tokens:2600,temperature:.65});
   const raw=typeof result?.response==='string'?JSON.parse(result.response.replace(/^```(?:json)?\s*|\s*```$/g,'')):result?.response;
   await this.serial(async()=>{const r=this.record,w=r.world,a=w.agents.find(a=>a.id===id),design=validateBlueprint(w,a,raw),project=adoptBlueprint(w,a,design,MODEL);r.ai.succeeded++;(r.designFailures??={})[id]=null;
  ...
  }catch(e){await this.serial(async()=>{const reason=String(e.message||'design_provider_error').slice(0,180);(this.record.designFailures??={})[id]=reason;const s=this.record.world.settlement;s.designs.push({at:this.now(),agentId:id,status:'rejected',model:MODEL,reason});s.designs=s.designs.slice(-30);await this.save();});}
 }
