...
export function economyCandidates(w,a){
 ...
 for(const node of w.resourceSites.nodes){if(!REGIONAL_ITEMS[node.item]||!node.knownBy.includes(a.id)||node.remaining<=0||a.inventory[node.item]>=2||!roomFor(w,a,node.item))continue;const destination=interactionPoint(w,a,node.position);if(!destination)continue;const useful=node.item==='flint'&&!a.inventory.sharpStone||node.item==='longFiber'&&(a.inventory.cordage||0)<2||node.item==='potteryClay'&&!a.inventory.firedVessel||node.item==='resin'&&w.settlement.projects.some(p=>p.ownerId===a.id&&p.status==='complete'&&p.parts.some(x=>x.kind==='roof'&&!x.sealant));if(useful)result.push({id:'regional:'+node.id,label:'Gather '+REGIONAL_ITEMS[node.item].name,score:34-gap(a.coordinates,destination)*.2,reasons:[['useful local material',34]],job:{kind:'gather',nodeId:node.id,item:node.item,quantity:2,position:node.position,destination,minutes:3}});}
