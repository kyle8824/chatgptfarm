...
const socialKinds=['conversation','hand_game'];
...
export function workFreeTime(w,a,t,minutes){
 ...
 if(!t.social){
  if(!ready(w,b,j.kind))return fail(`${b.name} is occupied or does not want this activity now.`);
  if(b.task)outcome(w,b,b.task,true,`${b.name} ends the quiet break to join ${a.name}.`,'superseded');
  t.social={rounds:[],scores:[0,0]};
  b.task={...structuredClone(t),id:t.id+':guest',decisionId:null,source:'fallback',model:null,fallbackReason:'accepted_social_invitation',decisionSummary:`I want to join ${a.name} for ${j.kind==='conversation'?'conversation':'a hand-sign game'}.`,origin:{...b.coordinates},destination:{...b.coordinates},path:[{...b.coordinates}],phase:'work',targetPosition:b.position,selected:{id:'social:guest:'+a.id,label:j.kind==='conversation'?'Talk with '+a.name:'Play a hand-sign game with '+a.name,job:{...j,destination:{...b.coordinates},partnerId:a.id,hostId:a.id,sessionId:t.id}},social:null};
 ...
 let detail;
 if(j.kind==='conversation'){shareMemory(w,a,b);shareMemory(w,b,a);detail=`${a.name} and ${b.name} spent time together and compared remembered experiences.`;}
 ...
 b.task.socialResult={success:true,detail};return {done:true,success:true,detail};
}
