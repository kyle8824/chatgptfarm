...
export function validateBlueprint(w,a,raw){
 ...
 for(const input of raw.parts){
  ...
  if(supports.some(s=>!touch(r,partBounds(p,s))))throw Error(`Disconnected support for ${part.id}`);
...
Choose a supplied available site. Coordinates are local world units relative to its center, bottom height 0 is ground; axis-aligned boxes only. 4–28 parts, total material units <=160. All sizes >=.08, tops <=4, parts within site's width/depth. Parts must rest on dry ground or touch an earlier declared supporting part; requires must list those touching supports. No floating geometry, no executable Javascript. Timber and stone can support; reeds only wall/roof. Cost per part=max(1,ceil(volume*5 timber, *8 stone, *3 reeds)); labor comes later.
Storage: horizontal deck floor <=.45 thick, at least 3 vertical walls enclosing/touching the floor, inside height >=.45. A full timber lid/roof and 4 timber/stone walls permit owner-only secured storage. Make doors/lids as box parts; visitors access containers from outside. Capacity follows dimensions.
