...
export function designContext(w,a){const projects=w.settlement.projects.filter(p=>knownPlace(w,a,p)),stores=w.settlement.stores.filter(s=>knownPlace(w,a,s)),trees=w.settlement.trees.filter(t=>knownPlace(w,a,t));return {person:[a.name,a.surname].filter(Boolean).join(' '),personId:a.id,allowedReplacementIds:projects.filter(p=>p.status==='complete'&&(p.ownerId===a.id||p.access==='shared')).map(p=>p.id),family:familyContext(w,a),comfort:comfortContext(w,a),remainingDesignSlots:Math.max(0,12-householdProjects(w,a).length),craft:craftSnapshot(a),techniqueGraph:CRAFT_GRAPH,needs:a.needs,carry:loadOf(w,a),inventory:a.inventory,propertyConcern:a.propertyConcern||0,memories:a.memories.slice(0,5).map(m=>m.text),stores:stores.map(s=>({name:s.name,ownerId:s.ownerId,access:s.access,secured:s.secured,load:loadOf(w,s),capacityKg:s.capacityKg,position:s.position})),projects:projects.map(p=>({id:p.id,name:p.name,performance:structurePerformance(p),observedProblems:Object.values(p.observations||{}).filter(o=>o.agentId===a.id),repairs:p.parts.reduce((n,x)=>n+(x.repairs||0),0),maintenanceRequested:p.maintenanceRequested||null,replacesProjectId:p.replacesProjectId||null,supersededBy:p.supersededBy||null,purpose:p.purpose,status:p.status,ownerId:p.ownerId,ownerName:w.agents.find(x=>x.id===p.ownerId)?.name||'Unknown',canReplace:p.status==='complete'&&(p.ownerId===a.id||p.access==='shared'),affordances:p.affordances?.labels||[],feedback:p.feedback?.observations?.[a.id]||null,code:p.ownerId===a.id?p.code?.slice(0,3000):undefined})),materialSources:['stone','reeds','clay'].flatMap(kind=>materialSources(w,kind,a).map(s=>({material:kind,position:s.position,remaining:s.remaining}))),resources:{...w.resources,standingTimber:trees.reduce((n,t)=>n+treeUnits(w,t),0)},sites:buildingSites(w,a).map(physicalSite)};}
...
Return JSON {build:boolean,name:string,purpose:string,siteId:string,access:"private"|"shared",rationale:string,code:string,replacesProjectId?:string}. purpose is YOUR short description, not an enum. code is a construction program in a bounded JavaScript subset: const/let, arithmetic, comparisons, if, for loops (<=128 iterations), plain named functions (<=8 nested calls), arrays (bounded local array.push is supported), template strings and plain objects. Math.min/max/abs/floor/ceil/round/sin/cos/sqrt and Math.PI are available. Variables site and materials are already supplied and expose real dimensions and currently gatherable totals. Read site.width/site.depth; use different names for your own dimensions. A local variable cannot change the actual site boundaries. No eval, imports, network, new, timers, global state, property mutation or arbitrary engine changes.
