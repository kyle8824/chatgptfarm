...
export function buildingSites(w,a){
 ...
 for(const x of [48,53,78,83,25,30]){const y=riverY(x),p={x,y:y+2.6};if(w.settlement.projects.some(b=>(b.purpose==='bridge'||b.spansWater)&&Math.abs(b.position.x-x)<4))continue;
...
export function designContext(w,a){return {person:a.name,family:familyContext(w,a),comfort:comfortContext(w,a),remainingDesignSlots:Math.max(0,12-w.settlement.projects.length),craft:craftSnapshot(a),techniqueGraph:CRAFT_GRAPH,needs:a.needs,carry:loadOf(w,a),inventory:a.inventory,propertyConcern:a.propertyConcern||0,memories:a.memories.slice(0,5).map(m=>m.text),stores:w.settlement.stores.map(s=>({name:s.name,ownerId:s.ownerId,access:s.access,secured:s.secured,load:loadOf(w,s),capacityKg:s.capacityKg,position:s.position})),projects:w.settlement.projects.map(p=>({id:p.id,name:p.name,performance:structurePerformance(p),observedProblems:Object.values(p.observations||{}).filter(o=>o.agentId===a.id),repairs:p.parts.reduce((n,x)=>n+(x.repairs||0),0),maintenanceRequested:p.maintenanceRequested||null,replacesProjectId:p.replacesProjectId||null,supersededBy:p.supersededBy||null,purpose:p.purpose,status:p.status,ownerId:p.ownerId,affordances:p.affordances?.labels||[],feedback:p.feedback?.observations?.[a.id]||null,code:p.ownerId===a.id?p.code?.slice(0,3000):undefined})),materialSources:['stone','reeds','clay'].flatMap(kind=>materialSources(w,kind,a).map(s=>({material:kind,position:s.position,remaining:s.remaining}))),resources:{...w.resources,standingTimber:w.settlement.trees.reduce((n,t)=>n+treeUnits(w,t),0)},sites:buildingSites(w,a).map(physicalSite)};}
...
export function validateBlueprint(w,a,raw,issuedSite=null){
 ...
 const replaced=raw.replacesProjectId?w.settlement.projects.find(p=>p.id===raw.replacesProjectId):null;if(raw.replacesProjectId&&(!replaced||replaced.status!=='complete'||replaced.ownerId!==a.id&&replaced.access!=='shared'))throw Error('Replacement must reference an accessible completed structure');
 ...
 for(const input of raw.parts){
  ...
  const center=vec(input.center,-3,4),size=vec(input.size,.08,6),part={id:input.id,kind:input.kind,material:input.material,center,size,shape:input.shape==='cylinder'?'cylinder':'box',requires:input.requires||[],finish:input.finish||'rough',built:false,workMinutes:0};
