...
export function validateBlueprint(w,a,raw){
 ...
 for(const input of raw.parts){
  ...
  const disconnected=supports.find(s=>!touch(r,partBounds(p,s)));if(disconnected){const b=partBounds(p,disconnected),gaps=[Math.max(r.minX-b.maxX,b.minX-r.maxX,0),Math.max(r.bottom-b.top,b.bottom-r.top,0),Math.max(r.minZ-b.maxZ,b.minZ-r.maxZ,0)];throw Error(`${part.id} does not touch ${disconnected.id}: gaps x/height/z=${gaps.map(n=>n.toFixed(2)).join('/')}; each must be <=0.16. Correct the centers or supporting dependency`);}
