...
const chest=world.settlement.stores.find(s=>s.id===project.storeId);world.resources.stones-=3;a.inventory.stones+=3;a.needs={hunger:95,hydration:95,energy:95,warmth:95};a.task=null;delete a.freeTime.practice;
// Check the actual deposit journey without assuming it outranks every new
// interest or improvement goal within a twelve-minute utility-policy window.
const deposit=settlementCandidates(world,a).find(c=>c.job?.kind==='deposit'&&c.job.storeId===chest.id);assert(deposit);
const hauling={actionId:deposit.id,selected:deposit,destination:deposit.job.destination,path:liveRoute(world,a.coordinates,deposit.job.destination,a),pathIndex:1,phase:'travel',requiredMinutes:.75,workMinutes:0};let arrived=false;
for(let i=0;i<1200;i++){const movement=advanceLiveRoute(world,a,hauling,.6);assert(!movement.blocked);if(movement.arrived){arrived=true;break;}}
assert(arrived,'supplies must be carried along a real route to the chest');assert(workSettlement(world,a,hauling,.75).success);assert.equal(chest.items.stones,3,'physical handling populates the completed chest');
