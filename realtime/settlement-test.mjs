...
const old=JSON.stringify(storage.settlement);ensureSettlement(storage);assert.equal(JSON.stringify(storage.settlement),old,'migration is idempotent');
...
taker.coordinates={x:42,y:36.15};taker.needs.hunger=3;taker.inventory.berries=0;owner.coordinates={x:42,y:38};const trust=relationship(storage,owner,taker).trust;
let choice=settlementCandidates(storage,taker).find(c=>c.job?.theft);assert(choice,'desperation can overcome an ownership norm');const theftTask={selected:choice,workMinutes:0,requiredMinutes:.5};taker.coordinates={...choice.job.destination};const theft=workSettlement(storage,taker,theftTask,.5);assert(theft.success);assert.equal(taker.inventory.berries,2);assert(relationship(storage,owner,taker).trust<trust,'witnessed theft changes trust');
...
for(let i=0;i<18000&&heavy.status!=='complete';i++)await step(loaded,.6);assert(heavy.status==='complete',JSON.stringify({task:builder.task,inventory:builder.inventory,stores:loaded.settlement.stores}));assert(loaded.settlement.stores.some(s=>(s.items.stones||0)>0),'unrelated cargo is stored to make room');
