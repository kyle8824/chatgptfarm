...
import {outcome,display,startTask,interrupt,work} from '../engine/persistent-actions.js';
...
import {liveUrgency,liveCandidates,retireObsoleteTask,resumeLiveTask,rememberFailure,shouldInterruptForNeed} from './behavior.mjs';
...
export async function step(w,seconds,{mind=null,fallbackReason='no_provider',wallTime=Date.now()}={}){
 ...
 for(const a of agents){
  ...
  if(shouldInterruptForNeed(w,a,urgent))interrupt(w,a,urgent);
  ...
  n.hydration=clamp(n.hydration-(a.inventory.firedVessel>0?5.5:6.5)*ratio);n.hunger=clamp(n.hunger-4.2*ratio*(a.life.pregnancy?1.2:1));n.energy=clamp(n.energy-2.8*ratio*(a.life.pregnancy?1.15:1)-(walked?3.6*ratio:0));
