...
try{for(const mobile of [true,false]){
 ...
 for(const kind of Object.keys(frames)){
  await page.evaluate(kind=>{window.currentStage=kind;valley.focus=null;const p=stageFrames[kind].settlement.projects[0];valley.controls.target.set(p.position.x,.9,p.position.y);valley.camera.position.set(p.position.x+4,3.8,p.position.y+5);valley.controls.update();document.querySelector('.eyebrow').textContent='ISOLATED CONSTRUCTION · '+kind.replaceAll('_',' ');},kind);
