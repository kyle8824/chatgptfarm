...
export function beginReturnRoute(from,goal,{arrivalRadius=8,precise=false}={}){return {precise,origin:{...from},goal:{...goal},arrivalRadius,nodes:{'0,0':{x:0,y:0,p:{...from},g:0}},open:[{key:'0,0',score:distance(from,goal)}],expanded:0};}
export function advanceReturnRoute(w,a,search,{maxExpansions=RETURN_ROUTE_EXPANSIONS}={}){
 ...
 while(search.open.length&&expanded<Math.min(RETURN_ROUTE_EXPANSIONS,maxExpansions)&&search.expanded<RETURN_ROUTE_LIMIT){
  ...
  if(distance(node.p,search.goal)<=radius&&(!search.precise||liveClear(w,node.p,search.goal,a))){const path=[];let n=node;while(n){path.unshift(n.p);n=search.nodes[n.previous];}if(search.precise&&distance(path.at(-1),search.goal)>1e-8)path.push({...search.goal});return {path,expanded};}
