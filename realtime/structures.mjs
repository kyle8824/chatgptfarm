...
export function onDeck(w,p){return (w.settlement?.projects||[]).some(b=>b.purpose==='bridge'&&b.parts.some(part=>{if(!part.built||part.kind!=='deck')return false;const r=partBounds(b,part);return p.x>=r.minX+.12&&p.x<=r.maxX-.12&&p.y>=r.minZ-.05&&p.y<=r.maxZ+.05;}));}
...
export function structureWaypoints(w){const points=[];for(const b of w.settlement?.projects||[]){
  ...
  if(b.purpose==='bridge'){for(const part of b.parts.filter(p=>p.kind==='deck'&&p.built)){const r=partBounds(b,part),x=(r.minX+r.maxX)/2;for(const y of [r.minZ,r.maxZ,(r.minZ+r.maxZ)/2])points.push({x,y});}points.push({x:b.position.x,y:bb.minZ-.5},{x:b.position.x,y:bb.maxZ+.5});}
 }return points;}
export function builtCover(w,p){return (w.settlement?.projects||[]).some(b=>b.status==='complete'&&b.purpose==='shelter'&&b.parts.some(part=>{if(part.kind!=='roof')return false;const r=partBounds(b,part);return p.x>=r.minX&&p.x<=r.maxX&&p.y>=r.minZ&&p.y<=r.maxZ;}));}
