...
export function coverEffectiveness(w,p){let protection=0;for(const b of w.settlement?.projects||[])if(b.purpose==='shelter'||b.affordances?.restPoints?.length)for(const part of b.parts){if(!activeParts(b).has(part.id)||part.kind!=='roof'||part.center[1]-part.size[1]/2<1.35)continue;const r=partBounds(b,part);if(p.x>=r.minX&&p.x<=r.maxX&&p.y>=r.minZ&&p.y<=r.maxZ)protection=Math.max(protection,surfaceProtection(part));}return protection;}
