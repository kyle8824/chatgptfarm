...
// A fresh query projection: never retain quantities across a transfer or harvest.
export function groundTimber(w){const totals=new Map();for(const b of w.wood.batches)if(b.holder.kind==='ground')totals.set(b.holder.id,(totals.get(b.holder.id)||0)+b.units);return totals;}
export function settlementFrame(w,{includeTrees=true}={}){
 const s=w.settlement;if(!s)return null;const timber=new Map();for(const b of w.wood.batches)if(b.holder.kind==='ground')timber.set(b.holder.id,(timber.get(b.holder.id)||0)+b.units);const remaining=t=>timber.get(t.id)||0;return {revision:s.revision,stores:s.stores.filter(x=>x.kind!=='pile'||Object.values(x.items).some(n=>n>0)).map(x=>({...x,load:loadOf(w,x)})),projects:s.projects.map(({code,stages,...p})=>({...p,...(stages?{stages:stages.map(({code,...stage})=>stage)}:{})})),treesPartial:!includeTrees,trees:s.trees.filter(t=>includeTrees||t.depleted||t.handGathered||t.timber!==t.initialUnits||remaining(t)<t.initialUnits).map(t=>({id:t.id,position:t.position,remaining:remaining(t),initialUnits:t.initialUnits,pine:t.pine,height:t.height})),designs:s.designs.slice(-6),incidents:s.incidents.slice(-8)};
}
