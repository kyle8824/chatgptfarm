...
let browser;
try{
 report.before=await read();await new Promise(r=>setTimeout(r,12000));report.unattended=await read();
 ...
 for(const[name,viewport]of [['desktop',{width:1440,height:960}],['mobile',{width:412,height:915}]]){
  ...
  await p.goto(base+'/live/',{waitUntil:'networkidle'});await p.getByText('Live world',{exact:true}).waitFor({timeout:30000});await p.waitForTimeout(6000);
