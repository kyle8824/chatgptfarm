...
let previous=null,total=0,max=0;for(let i=0;i<100;i++){await step(c.record.world,.6);c.record.revision++;const next=JSON.parse(JSON.stringify(c.frame(1,{includeTrees:false}))),patch=JSON.parse(JSON.stringify(frameDelta(previous,next))),restored=applyFrameDelta(previous,patch);assert.deepEqual(restored,next);if(i){const bytes=JSON.stringify(patch).length;total+=bytes;max=Math.max(max,bytes);}previous=structuredClone(next);}
const original=c.frame(1,{includeTrees:false}),snapshot=structuredClone(original),id=c.record.world.agents[0].id;c.record.world.agents[0].needs.hunger=12;c.record.world.agents[0].inventory.resin=3;c.record.revision++;const latest=c.frame(1,{includeTrees:false}),changed=applyFrameDelta(snapshot,frameDelta(snapshot,latest));assert.equal(changed.agents.find(a=>a.id===id).needs.hunger,12);assert.equal(changed.agents.find(a=>a.id===id).inventory.resin,3);
