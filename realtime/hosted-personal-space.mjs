...
try{
 for(const [name,viewport] of [['desktop',{width:1440,height:960}],['mobile',{width:412,height:915}]]){
  ...
  assert(last.agents.some(a=>a.task?.phase==='travel'||a.task?.workMinutes!==first.agents.find(b=>b.id===a.id).task?.workMinutes),'real tasks progress even when resting');assert.deepEqual(errors,[]);
  report.views.push({name,frames:frames.length,minimumSeparation:clearance,first,last,errors});await ctx.close();
 }
