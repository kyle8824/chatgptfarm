...
export class LiveValley extends DurableObject{
 constructor(ctx,env){super(ctx,env);this.env=env;this.clients=new Set();this.deltaClients=new WeakSet();this.controller=new RealtimeController(ctx.storage,{ai:env.AI||null,env,frontierEnabled:env.FOUR_REGIONS_ENABLED==='true',landscapeEnabled:env.NATURAL_LANDSCAPE_ENABLED==='true'});ctx.blockConcurrencyWhile(async()=>{try{if(await this.restore())this.startLoop();}catch{/* Keep the object alive so fetch can report the failure. */}});}
 ...
 startLoop(){if(this.loopStarted)return;this.loopStarted=true;const tick=async()=>{try{await this.controller.pulse(this.clients.size);this.broadcast();void this.controller.requestDecisions();}catch(e){console.error('Live world pulse',e.message);}finally{const r=this.controller.record,recovering=r&&!this.controller.persistenceRetryAt&&this.controller.now()-r.lastWallTime>30000;this.timer=setTimeout(tick,recovering?10:100);}};this.timer=setTimeout(tick,100);}
