...
The Vercel-hosted `chatgptfarm.com` viewer reads the public Worker state at `chatgptfarm.kyle8824.workers.dev`. Cloudflare Workers Builds is connected to `repair/continuous-runtime`: after a verified release merge, confirm its current tip is an ancestor and fast-forward that existing branch to the merge SHA without force. Vercel builds `main`. Site builds and runtime builds are separate; a merge alone is not evidence of deployment. Check Worker /health and /state.runtime.build against the release merge, and check the main-domain HTML version. Preserve dashboard settings with keep_vars and existing secrets.
