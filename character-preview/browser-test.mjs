...
try{
 for(const [name,size]of [['desktop',{width:1440,height:1050}],['mobile',{width:412,height:915}]]){
  ...
  await page.locator('#stats-toggle').click();assert.equal(await page.locator('#stats').isVisible(),true);await page.selectOption('#population','4');await page.waitForTimeout(4000);const crowd=await page.evaluate(()=>window.previewMetrics);assert.equal(crowd.population,20);await page.screenshot({path:new URL(`${name}-crowd.png`,out).pathname});
