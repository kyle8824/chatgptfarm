...
Static pieces are merged per moving joint into a shared vertex-color material. The initial budgets are 18,000 triangles and 16 meshes per person and 10,000 triangles and 16 meshes per deer. The current two people plus deer use roughly 38,000 model triangles and 42 model draw calls before shadow passes. Lighting, vegetation, shadow passes and pixel density also cost GPU time and must be included when evaluating performance.
