...
export function worldFailure(error,now=Date.now()){
 ...
 const quota=/exceeded.*(?:limit|quota)|(?:limit|quota).*exceeded|daily.*limit|too many.*(?:rows|requests)/i.test(detail);
 const full=/SQLITE_FULL|database or disk is full/i.test(detail);
 return {code:quota?'hosting_limit':full?'storage_full':/checkpoint|gzip|decompress/i.test(detail)?'checkpoint_unavailable':'world_unavailable',error:quota?'The world’s hosting limit has been reached. The server cannot run the world right now.':full?'The world’s storage is full. The server cannot save new progress.':'The world server is temporarily unavailable.',detail,retryAfterSeconds:quota&&/daily/i.test(detail)?Math.ceil((Date.UTC(new Date(now).getUTCFullYear(),new Date(now).getUTCMonth(),new Date(now).getUTCDate()+1)-now)/1000):30};
}
