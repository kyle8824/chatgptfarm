...
const defaultEnvironment = () => typeof process !== "undefined" ? process.env : {};
...
function parseDecision(parsed, context, data, model) {
...
export function createOpenAIMind(env = defaultEnvironment()) {
  const model=env.OPENAI_MODEL || DEFAULT_MODEL;
  const enabled = !!env.OPENAI_API_KEY;
  return {enabled,model,async decide(context) { return callDecision(context, {}, env); },async replay(context, { originalActionId, removedMemoryId }) {return callDecision(context, { replayNote: `Decision DNA replay: the same decision is being reconsidered with memory ${removedMemoryId} removed. The original selected descriptor was ${originalActionId}. Choose naturally from the remaining evidence; do not intentionally preserve or change it.` }, env);},async reflect({ agent, events, memories }) {if (!enabled) return null;const response = await fetch(API_URL, {signal: AbortSignal.timeout(10000),method: "POST",headers: { "Authorization": `Bearer ${env.OPENAI_API_KEY}`, "Content-Type": "application/json" },body: JSON.stringify({model,reasoning: { effort: "low" },instructions: `Create one concise explicit reflection for an autonomous person's persistent memory. Infer a useful belief from only the supplied experiences. Do not invent events, outcomes, recipes, or hidden facts. Do not output chain-of-thought.`,input: JSON.stringify({ name: agent.name, traits: agent.traits, current_goal: agent.mind?.currentGoal, recent_events: events, recent_memories: memories }),text: { format: { type: "json_schema", name: "chatgptfarm_reflection_v07", strict: true, schema: reflectionSchema } },max_output_tokens: 360})});if (!response.ok) throw new Error(`Reflection API ${response.status}: ${(await response.text()).slice(0, 180)}`);const data = await response.json(), text = outputText(data);if (!text) throw new Error(`Reflection response contained no text${data?.incomplete_details?.reason ? ` (${data.incomplete_details.reason})` : ''}`);const parsed=JSON.parse(text);parsed._usage=data.usage||null;return parsed;}}
}