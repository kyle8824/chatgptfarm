...
export function importLegacyWood(world){
  ...
  const ledger={version:1,rulesVersion:WOOD_RULES.version,lastHour:at,nextId:1,batches:[],sinks:[],
  ...
  for(const entry of holders)for(const key of ['dryWood','wetWood']){
    ...
    ledger.batches.push({id:`wood-${ledger.nextId++}`,holder:entry.holder,units:count,dryKg,
...
export function projectWoodCounts(ledger,location){
  ...
  for(const b of ledger.batches)if(sameHolder(b.holder,location))result[b.waterKg/b.dryKg<=WOOD_RULES.fuelWaterRatio?'dryWood':'wetWood']+=b.units;
  return result;
}
...
export function consumeWood(ledger,id,count,{reason,operationId}={}){
  ...
  const fraction=count/b.units,record={operationId,batchId:id,holder:copy(b.holder),units:count,dryKg:b.dryKg*fraction,waterKg:b.waterKg*fraction,reason,at:ledger.lastHour};
