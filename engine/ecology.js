...
export const ECOLOGY_VERSION='living-basin-1.4.1';
...
const SPECIES={
 deer:{step:3.35,fearRadius:17,panicRadius:7.5,traceChance:.26,traceSize:1},
 rabbit:{step:2.2,fearRadius:10,panicRadius:4.2,traceChance:.11,traceSize:.48},
 bear:{step:3.05,fearRadius:24,panicRadius:9,traceChance:.34,traceSize:1.45},
 fish:{step:4.2,fearRadius:0,panicRadius:0,traceChance:0,traceSize:0}
};
function animal(id,species,label,x,y,extra={}){return{id,species,label,position:{x,y},previousPosition:{x,y},target:{x,y},home:{x,y},activity:'rest',behavior:'calm',behaviorUntil:0,behaviorGoal:{x,y},fear:0,needs:{energy:80,hunger:72,thirst:75},active:true,localPresence:true,rangeState:'basin',rangeUntil:null,ageClass:'adult',history:[],ai:{eligible:true,mode:'simulation',calls:0,lastDecisionAt:null},movement:{from:{x,y},to:{x,y},worldDay:1,worldHour:6},...extra}}
function ensureArrays(e){e.events||=[];e.traces||=[];e.signs||=[];e.eventCooldowns||={}}
function channelPoints(w){const o=w?.worldModel?.objects?.find(x=>x.type==='creek_segment');return o?.geometry?.points?.map(([x,y])=>({x,y}))||creekPoints}
function projectChannel(w,p){const points=channelPoints(w);let best=null,total=0;for(let i=1;i<points.length;i++){const a=points[i-1],b=points[i],dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy),t=clamp(((p.x-a.x)*dx+(p.y-a.y)*dy)/(len*len||1),0,1),q={x:a.x+t*dx,y:a.y+t*dy},distance=dist(p,q);if(!best||distance<best.distance)best={point:q,distance,along:total+t*len};total+=len}return{...best,length:total}}
function channelAt(w,along){const points=channelPoints(w);for(let i=1;i<points.length;i++){const a=points[i-1],b=points[i],len=dist(a,b);if(along<=len){const t=clamp(along/(len||1),0,1);return{x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t}}along-=len}return{...points.at(-1)}}
function nearestCreek(p,w){return projectChannel(w,p).point}
function worldStamp(w){return w.day*24+w.hour}
function surfaceWetness(w){return clamp(Number(w.environmentState?.surfaceWetness??(w.weather==='rain'?.82:.32)),.08,1)}
function segmentOfDay(w){if(w.hour>=5&&w.hour<=8)return'dawn';if(w.hour>=17&&w.hour<=21)return'dusk';if(w.hour>=9&&w.hour<17)return'day';return'night'}
function activityDrive(w,a){const seg=segmentOfDay(w),rain=w.weather==='rain';let d=.5;if(a.species==='deer')d=seg==='dawn'||seg==='dusk'?1:seg==='night'?.52:.34;else if(a.species==='rabbit')d=seg==='dawn'||seg==='dusk'?1:seg==='night'?.56:.18;else if(a.species==='bear')d=seg==='dawn'||seg==='dusk'?.78:seg==='night'?.56:.46;else if(a.species==='fish')d=seg==='dawn'||seg==='dusk'?.9:.62;if(rain)d*=a.species==='rabbit'?.45:a.species==='deer'?.76:.9;return clamp(d,0,1)}
...
function traceSuitability(w,p){const d=dist(p,nearestCreek(p,w)),wet=surfaceWetness(w);let s=d<2.5?.9:d<5?.64:d<9?.34:.11;s*=.68+wet*.7;if(w.weather==='rain')s*=d<7?1.08:.72;return clamp(s,.04,1)}
function addTrace(w,a,from,to){const s=SPECIES[a.species],moved=dist(from,to);if(!s?.traceChance||a.species==='fish'||moved<.85)return;const suitability=traceSuitability(w,from),panic=a.activity==='flee'?1.35:1,chance=Math.min(.72,s.traceChance*suitability*panic);if(unit(w,`${a.id}:trace:${a.history.length}`)>chance)return;const e=w.ecologySystem,m={x:from.x+(to.x-from.x)*.58,y:from.y+(to.y-from.y)*.58},id=`TRACE-${a.id}-${w.day}-${String(w.hour).padStart(2,'0')}`;if(e.traces.some(x=>x.id===id))return;const creekD=dist(m,nearestCreek(m,w)),wet=surfaceWetness(w),substrate=creekD<4||(wet>.72&&creekD<9)?'mud':creekD<8||wet>.52?'soft-ground':'forest-floor';e.traces.unshift({id,sourceId:a.id,species:a.species,label:`${a.species} tracks`,position:bound(m),heading:heading(from,to),clarity:substrate==='mud'?.9:substrate==='soft-ground'?.72:.55,size:s.traceSize,substrate,ageHours:0,created:{day:w.day,hour:w.hour},active:true});e.traces=e.traces.slice(0,24)}
...
function ageSigns(w){const rain=w.weather==='rain';for(const s of w.ecologySystem.signs){s.ageHours=(s.ageHours||0)+1;s.clarity=clamp((s.clarity??.7)-(rain ? .11 : .038),0,1);if(s.ageHours>30||s.clarity<.18)s.active=false}w.ecologySystem.signs=w.ecologySystem.signs.filter(x=>x.active).slice(0,18)}
...
function updateNeeds(a){const moving=['move','flee','swim'].includes(a.activity);if(a.species==='rabbit'){a.needs.hunger=clamp(a.needs.hunger-1.55);a.needs.thirst=clamp(a.needs.thirst-.35)}else if(a.species==='bear'){a.needs.hunger=clamp(a.needs.hunger-1.8);a.needs.thirst=clamp(a.needs.thirst-1.05)}else if(a.species==='deer'){a.needs.hunger=clamp(a.needs.hunger-1.65);a.needs.thirst=clamp(a.needs.thirst-1.25)}else{a.needs.hunger=clamp(a.needs.hunger-.8);a.needs.thirst=clamp(a.needs.thirst-.5)}a.needs.energy=clamp(a.needs.energy-(a.activity==='flee'?5:moving?1.8:.5));if(a.activity==='rest'||a.activity==='hide')a.needs.energy=clamp(a.needs.energy+4.5)}
function persistRoutine(w,a){if((a.behaviorUntil||0)<=worldStamp(w)||!a.behaviorGoal)return null;if(a.activity==='rest'||a.activity==='hide'||a.activity==='freeze'||a.activity==='drink')return jitter(w,a,a.position,a.species==='rabbit'?.25:.55,'persist-still');if(a.activity==='graze'||a.activity==='forage')return jitter(w,a,a.behaviorGoal,a.species==='rabbit'?1.2:2.3,'persist-feed');return a.behaviorGoal}
function decideAnimal(w,a){
 ...
 if(a.species==='fish'){const c=projectChannel(w,a.position);a.channelDirection??=(hash(a.id)%2?1:-1);let next=c.along+a.channelDirection*s.step;if(next<1||next>c.length-1){a.channelDirection*=-1;next=clamp(c.along+a.channelDirection*s.step,1,c.length-1)}return hold(w,a,{activity:'swim',behavior:'following creek channel',goal:channelAt(w,next),hours:1})}
 ...
 if(a.species!=='rabbit'&&a.needs.thirst<38){const water=nearestCreek(a.position,w);if(dist(a.position,water)<2.6){a.needs.thirst=clamp(a.needs.thirst+26);return hold(w,a,{activity:'drink',behavior:'drinking',goal:a.position,hours:1})}return hold(w,a,{activity:'move',behavior:'seeking water',goal:jitter(w,a,water,1,'water'),hours:2})}
 if(a.species==='deer'&&a.fear<25&&a.needs.energy>=36){const companions=w.ecologySystem.wildlife.filter(x=>x.id!==a.id&&x.species==='deer'&&x.active&&x.localPresence!==false&&x.fear<25&&dist(a.position,x.position)<14&&['graze','move'].includes(x.activity));if(companions.length){const other=companions.sort((x,y)=>dist(a.position,x.position)-dist(a.position,y.position))[0],separation=dist(a.position,other.position);if(separation>4)return hold(w,a,{activity:'move',behavior:'keeping near nearby deer',goal:jitter(w,a,other.position,1.5,'associate'),hours:1})}}
 const persisted=persistRoutine(w,a);if(persisted)return persisted;
 const r=unit(w,`${a.id}:behavior`),home=a.home||a.position;
 if(a.needs.energy<36||drive<.24){const cover=a.species==='rabbit'?chooseHabitat(w,a):a.species==='deer'?chooseHabitat(w,a):home;return hold(w,a,{activity:a.species==='rabbit'?'hide':'rest',behavior:a.species==='rabbit'?'resting in cover':'resting',goal:jitter(w,a,cover,a.species==='rabbit'?.8:1.8,'rest'),hours:2+Math.floor(unit(w,`${a.id}:rest-hours`)*3)})}
 if(a.needs.hunger<48||r<.5+drive*.22){const food=chooseHabitat(w,a);a.needs.hunger=clamp(a.needs.hunger+(a.species==='bear'?6:7));return hold(w,a,{activity:a.species==='bear'?'forage':'graze',behavior:a.species==='bear'?'foraging':'feeding',goal:jitter(w,a,food,a.species==='rabbit'?1.4:a.species==='deer'?2.8:5,'feed'),hours:2+Math.floor(unit(w,`${a.id}:feed-hours`)*3)})}
 // Roaming is deliberately uncommon outside active periods. Rabbits stay tight to cover; bears range much farther.
 if(r<.62+drive*.26){const base=a.species==='rabbit'?chooseHabitat(w,a):a.species==='bear'?chooseHabitat(w,a):chooseHabitat(w,a),scale=a.species==='rabbit'?2.1:a.species==='bear'?7:4;return hold(w,a,{activity:'move',behavior:'roaming',goal:jitter(w,a,base,scale,'roam'),hours:2+Math.floor(unit(w,`${a.id}:roam-hours`)*2)})}
 return hold(w,a,{activity:a.species==='rabbit'?'hide':'rest',behavior:a.species==='rabbit'?'resting in cover':'resting',goal:jitter(w,a,home,a.species==='rabbit'?.7:1.5,'quiet'),hours:2})
}
function updateLocalPresence(w,a){
...
export function ensureEcology(w){
 ...
 if(!w.ecologySystem.wildlife.length){w.ecologySystem.wildlife=[
  ...
  animal('W-FISH-001','fish','creek fish school',15,18.5,{ageClass:'school'}),animal('W-FISH-002','fish','creek fish school',67,23,{ageClass:'school'})
 ];}
 for(const x of w.ecologySystem.wildlife){x.previousPosition||={...x.position};x.target||={...x.position};x.home||={...x.position};x.behaviorGoal||={...x.target};x.behaviorUntil??=0;x.localPresence??=true;x.rangeState||=(x.localPresence===false?'outside-basin':'basin');x.rangeUntil??=worldStamp(w)+(x.species==='bear'?4:x.species==='deer'?6:99999);x.movement||={from:{...x.position},to:{...x.position},worldDay:w.day,worldHour:w.hour};x.ai||={eligible:true,mode:'simulation',calls:0,lastDecisionAt:null};x.history||=[];if(x.species==='fish'&&((x.position?.y||0)>50||(x.target?.y||0)>50||(x.movement?.to?.y||0)>50)){const q=creekPoints[hash(x.id)%creekPoints.length];x.position={...q};x.previousPosition={...q};x.target={...q};x.home={...q};x.behaviorGoal={...q};x.movement={from:{...q},to:{...q},goal:{...q},worldDay:w.day,worldHour:w.hour,speed:0};x.history.push({day:w.day,hour:w.hour,activity:'migration',behavior:'coordinate migration',from:null,to:{...q},goal:{...q}})}else if(x.behavior==='seeking water'&&(x.target?.y||0)>50){const q=nearestCreek(x.position,w);x.target={...q};x.behaviorGoal={...q};x.movement={...x.movement,to:{...x.position},goal:{...q}}}}
 for(const a of w.ecologySystem.wildlife){if(a.species!=='fish')continue;const p=projectChannel(w,a.position);if(p.distance>.1){const from={...a.position};a.position={...p.point};a.previousPosition={...p.point};a.target={...p.point};a.behaviorGoal={...p.point};a.movement={from:{...p.point},to:{...p.point},worldDay:w.day,worldHour:w.hour,speed:0};a.history.push({day:w.day,hour:w.hour,activity:'migration',behavior:'aligned to canonical creek geometry',from,to:{...p.point}});a.history=a.history.slice(-48)}}
 updateAmbient(w);return w.ecologySystem;
}
export function advanceEcology(w){ensureEcology(w);ageTraces(w);ageSigns(w);for(const a of w.ecologySystem.wildlife){if(!a.active)continue;if(!updateLocalPresence(w,a))continue;updateNeeds(a);const prevBehavior=a.behavior,from={...a.position},goal=decideAnimal(w,a),base=SPECIES[a.species]?.step||3;let mult=1;if(a.activity==='flee')mult=1.75;else if(['rest','hide','freeze','drink'].includes(a.activity))mult=.08;else if(a.activity==='graze'||a.activity==='forage')mult=.42;else if(a.species==='rabbit')mult=.68;const to=moveToward(from,goal,base*mult);a.previousPosition=from;a.target=goal;a.position=to;a.movement={from,to,goal,worldDay:w.day,worldHour:w.hour,speed:base*mult};addTrace(w,a,from,to);addActivitySign(w,a);a.history.push({day:w.day,hour:w.hour,activity:a.activity,behavior:a.behavior,from,to,goal});if(a.history.length>48)a.history=a.history.slice(-48);if(prevBehavior!==a.behavior&&['feeding','foraging','resting'].includes(a.behavior)&&unit(w,`${a.id}:quiet-event`)<.07)ecoEvent(w,{kind:'wildlife-routine',title:`${a.label} is ${a.behavior}`,detail:`A routine wildlife behavior emerged from its needs, time of day and habitat.`,animal:a,importance:2,cooldown:18});a.ai.mode=(w.settings.ai.wildlife.individuals?.[a.id]??w.settings.ai.wildlife.enabled)?'eligible-ai':'simulation'}updateAmbient(w);w.ecologySystem.lastAdvanced={day:w.day,hour:w.hour};return w.ecologySystem}
function detectionRadius(w,x,radius){let r=radius;if(x.species==='rabbit')r=Math.min(r,['hide','freeze','rest'].includes(x.activity)?5.5:10);else if(x.species==='deer')r=Math.min(r,x.activity==='rest'?12:18);else if(x.species==='bear')r=Math.min(r,20);if(w.weather==='rain')r*=.82;return r}
export function visibleWildlifeForAgent(w,agent,radius=18){const c=agent.coordinates;if(!c)return[];return w.ecologySystem.wildlife.filter(x=>x.active&&x.localPresence!==false&&dist(c,x.position)<=detectionRadius(w,x,radius)).map(x=>({id:x.id,species:x.species,label:x.label,distance:Math.round(dist(c,x.position)*10)/10,behavior:x.behavior,activity:x.activity,fear:Math.round(x.fear)})).sort((a,b)=>a.distance-b.distance)}
export function visibleWildlifeTracesForAgent(w,agent,radius=12){const c=agent.coordinates;if(!c)return[];return w.ecologySystem.traces.filter(x=>x.active&&(x.clarity??0)>=.32&&dist(c,x.position)<=radius).map(x=>({id:x.id,sourceId:x.sourceId,species:x.species,label:x.label,distance:Math.round(dist(c,x.position)*10)/10,clarity:Math.round((x.clarity||0)*100)/100,heading:x.heading,ageHours:x.ageHours,substrate:x.substrate||null})).sort((a,b)=>a.distance-b.distance)}
export function visibleWildlifeSignsForAgent(w,agent,radius=10){const c=agent.coordinates;if(!c)return[];return w.ecologySystem.signs.filter(x=>x.active&&(x.clarity??0)>=.34&&dist(c,x.position)<=radius).map(x=>({id:x.id,sourceId:x.sourceId,species:x.species,kind:x.kind,label:x.label,distance:Math.round(dist(c,x.position)*10)/10,clarity:Math.round((x.clarity||0)*100)/100,ageHours:x.ageHours})).sort((a,b)=>a.distance-b.distance)}
export function wildlifeAIEnabled(w,id=null){if(id&&w.settings.ai.wildlife.individuals?.[id]!=null)return!!w.settings.ai.wildlife.individuals[id];return!!w.settings?.ai?.wildlife?.enabled}
