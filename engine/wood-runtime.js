...
export function projectWood(w){
  if(!w.wood)return;
  Object.assign(w.resources,projectWoodCounts(w.wood,ground('log')));
...
export function gatherWood(w,a,key,count,location='log'){
  return woodCommand(w,a,{from:ground(location),quantity:count,category:key,reason:'gather available branches'});
}
export function fuelFire(w,a,count=Math.min(2,a.inventory.dryWood)){
  woodCommand(w,a,{to:{kind:'stored',id:'camp-fire'},quantity:count,category:'dryWood',outputForm:'fuel',reason:'load fire with carried fuel'});
  w.structures.fire=true;
}
export function shelterWood(w,a){return woodCommand(w,a,{to:{kind:'stored',id:'camp-shelter'},quantity:4,category:'dryWood',outputForm:'shelter-component',reason:'retain wood in shelter'});}
...
export function woodConditions(w,h){
  ...
  const covered=!!w.structures.shelter&&['camp','camp-shelter','camp-drying'].includes(location);
...
export function advanceWood(w,toHour){
  ...
  for(const b of [...ledger.batches].filter(b=>b.form==='fuel')){
    ...
    ledger.sinks.push(sink);b.dryKg-=used;b.waterKg-=sink.waterKg;b.units-=sink.units;demand-=used;
    if(b.dryKg<1e-10)ledger.batches=ledger.batches.filter(x=>x!==b);
  }
  w.structures.fire=ledger.batches.some(b=>b.form==='fuel')||(ledger.legacyFireUntil??-1)>toHour;
  projectWood(w);
}
export function storeWood(w,a){
  ...
  a.position='camp';
  if(a.inventory.wetWood)woodCommand(w,a,{to:{kind:'stored',id:'camp-drying'},quantity:a.inventory.wetWood,category:'wetWood',reason:'place damp wood beneath existing shelter'});
  if(a.inventory.dryWood>retain)woodCommand(w,a,{to:{kind:'stored',id:'camp-drying'},quantity:a.inventory.dryWood-retain,category:'dryWood',reason:'protect spare fuel beneath existing shelter'});
  a.fuelStoreKnown=true;return true;
}
export function storedWood(w){return w.wood?projectWoodCounts(w.wood,{kind:'stored',id:'camp-drying'}):{dryWood:0,wetWood:0};}
...
export function dryWoodByFire(w,a){
  ...
  const hour=now(w),used=ledger.dryingHeat?.hour===hour?ledger.dryingHeat.waterKg:0;
  let budget=Math.max(0,.30-used);if(!budget)return false;
  if(!a.inventory.wetWood&&a.position==='camp'&&storedWood(w).wetWood)woodCommand(w,a,{from:{kind:'stored',id:'camp-drying'},quantity:1,category:'wetWood',reason:'take one stored branch to dry near fire'});
  ...
  ledger.dryingHeat={hour,waterKg:used+evaporated};
...
export function collectStoredFuel(w,a){
  ...
  a.position='camp';woodCommand(w,a,{from:{kind:'stored',id:'camp-drying'},quantity:count,category:'dryWood',reason:'retrieve dried wood from camp'});return true;
}
