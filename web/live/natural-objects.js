...
export class NaturalObjects{
 ...
 update(frame){
  ...
  for(const o of list){const signature=JSON.stringify([o.type,o.remaining,o.height,o.mapped]),old=this.objects.get(o.id);if(old?.signature===signature)continue;if(old)this.scene.remove(old.group);
   ...
   if(o.type==='log'&&o.id!=='OBJ-TREE-001'&&o.remaining>0){const m=piece(g,cylinder,'#775638',[0,.3,0],[.32,2.8*Math.min(1,o.remaining/18),.32]);m.rotation.z=Math.PI/2;}
   if(o.mapped||extraBerry){
    const count=o.type==='berries'?4:Math.max(0,Math.min(12,Math.ceil(o.remaining/2)));
    for(let i=0;i<count;i++){
     ...
     if(['stones','flint','resin'].includes(o.type)){const size=(.19+(i%3)*.07)*Math.cbrt(Math.min(1,Math.max(0,o.remaining/2-i)))*Math.cbrt(Math.max(1,o.remaining/24)); const m=piece(g,sphere,o.type==='resin'?'#b88938':o.type==='flint'?'#5e6668':i%2?'#939c8b':'#a6ac98',[x,y+size*.55,z],[size*1.3,size*.7,size]);m.rotation.y=ang;}
     else if(['reeds','longFiber'].includes(o.type)){if(!o.remaining)continue;for(let j=0;j<3;j++){const h=(.6+(i%3)*.15)*Math.min(1,Math.max(.2,o.remaining/2-i));piece(g,cylinder,'#819155',[x+j*.1,y+h/2,z],[.014,h,.014]);piece(g,cylinder,'#7d6844',[x+j*.1,y+h*.93,z],[.035,.2,.035]);}}
     else if(['clay','potteryClay'].includes(o.type)){const size=Math.cbrt(Math.min(1,Math.max(0,o.remaining/2-i))*Math.max(1,o.remaining/24));piece(g,sphere,'#a47b57',[x,y+.02,z],[.45*size,.09*size,.32*size]);}
     else if(o.type==='berries'){piece(g,sphere,'#446b45',[x,y+.34,z],[.55,.45,.5]);for(let j=0;j<3&&i*3+j<o.remaining;j++)piece(g,sphere,'#ad5247',[x+(j-1)*.19,y+.72,z+.12],[.07,.07,.07]);}
    }
   }
