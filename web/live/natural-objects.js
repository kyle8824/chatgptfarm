...
export class NaturalObjects{
 ...
 update(frame){
  ...
  for(const o of list){const signature=JSON.stringify([o.type,o.remaining,o.height,o.mapped]),old=this.objects.get(o.id);if(old?.signature===signature)continue;if(old)this.scene.remove(old.group);
   ...
   if(o.type==='water'){
    for(let i=1;i<CREEK_POINTS.length;i++){const [x,z]=CREEK_POINTS[i-1],[bx,bz]=CREEK_POINTS[i],length=Math.hypot(bx-x,bz-z),m=new T.Mesh(box,invisible);m.position.set((x+bx)/2-o.position.x,-.13-g.position.y,(z+bz)/2-o.position.y);m.scale.set(length,.06,2.5);m.rotation.y=-Math.atan2(bz-z,bx-x);g.add(m);}continue;
   }
   const extraBerry=o.type==='berries'&&o.id.startsWith('willow-basin:');
   if(o.mapped||extraBerry){
    ...
    for(let i=0;i<count;i++){
     ...
     if(o.type==='stones'){const size=o.remaining?.19+(i%3)*.07:.06;const m=piece(g,sphere,i%2?'#939c8b':'#a6ac98',[x,y+size*.55,z],[size*1.3,size*.7,size]);m.rotation.y=ang;}
     else if(o.type==='reeds'){if(!o.remaining)continue;for(let j=0;j<3;j++){const h=.6+(i%3)*.15;piece(g,cylinder,'#819155',[x+j*.1,y+h/2,z],[.014,h,.014]);piece(g,cylinder,'#7d6844',[x+j*.1,y+h*.93,z],[.035,.2,.035]);}}
     else if(o.type==='clay')piece(g,sphere,'#a47b57',[x,y+.02,z],[.45,.09,.32]);
