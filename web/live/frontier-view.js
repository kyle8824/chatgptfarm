...
export function installFrontier(view,frame){
 ...
 for(const home of frame.frontier.homes.filter(h=>h.id!=='willow-basin')){
  ...
  if(frame.frontier.landscapeVersion<2||!frame.frontier.landscapeVersion)view.scene.add(riverMesh(CREEK_POINTS.map(([x,z])=>[x+home.x-64,z+home.y-34]),view.waterMat));
 }
