...
import {landHeight,waterSample,STREAMS,LAKES,smooth} from '../../shared/landscape.js';
...
export function naturalTerrain(){
 ...
 for(let i=0;i<p.count;i++){
  const spread=v=>Math.abs(v)<=265?v:Math.sign(v)*(265+(Math.abs(v)-265)*5),x=spread(p.getX(i))-150,z=spread(p.getZ(i))-150,h=landHeight(x,z),water=waterSample(x,z),slope=Math.hypot(landHeight(x+.7,z)-h,landHeight(x,z+.7)-h)/.7;
...
export function installNaturalWater(view){
 ...
 const mist=new T.BufferGeometry(),v=[];for(let i=0;i<50;i++)v.push(-218+Math.sin(i*2.39)*1.7,4.6+(i%9)*.13,-216.8+Math.cos(i*1.77)*1.5);mist.setAttribute('position',new T.Float32BufferAttribute(v,3));view.scene.add(new T.Points(mist,new T.PointsMaterial({color:'#e5f0e7',size:.16,transparent:true,opacity:.35,depthWrite:false})));
