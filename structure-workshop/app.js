import {constructionPrograms} from './catalog.js';
import {structurePerformance,conditionOf} from '../shared/structure-performance.js';
import {CRAFT_GRAPH,constructionSpec,constructionBlockers,bindingLength,bindingBudget} from '../shared/craft.js';
...
function renderInfo(){
 ...
 $('#materials').innerHTML=resourceRows(p,isExample?[]:catalog?.stores).map(r=>`<tr><td>${esc(r.material)}</td><td>${Number(r.required.toFixed(3))}</td><td>${Number(r.committed.toFixed(3))}</td><td>${Number(r.atSite.toFixed(3))}</td><td>${Number(r.missing.toFixed(3))}</td></tr>`).join('');
 const performance=structurePerformance(p);
 $('#performance').innerHTML=`<h3>Condition & usefulness</h3><p>${isExample?'Illustrative condition':performance.recorded?'Recorded workmanship':'Earlier workmanship uses an explicit estimate'} · ${performance.status}</p><p>Weakest built component: ${Math.round(performance.condition*100)}%${performance.cargoKg!==null?'<br>Cargo allowance: '+performance.cargoKg.toFixed(1)+' kg':''}</p><p>Current roof protection: ${Math.round(performance.roofProtection*100)}%<br>Current supply capacity: ${Math.round(performance.storageFactor*100)}% of the design maximum</p><p class="muted">Weather and use cause wear. Repairs consume supplies and labor. Failed components stop supporting their dependent pieces.</p>${p.replacesProjectId?'<p>Designed to replace '+esc(catalog?.projects.find(x=>x.id===p.replacesProjectId)?.name||p.replacesProjectId)+'.</p>':''}${p.supersededBy?'<p>A replacement has been built. This structure and its supplies remain in the world.</p>':''}${Object.values(p.observations||{}).length?'<h3>What villagers noticed</h3><ul>'+Object.values(p.observations).slice(-4).map(o=>'<li>'+esc(o.detail)+'</li>').join('')+'</ul>':''}`;
 const designer=catalog?.agents?.find(a=>a.id===p.ownerId),specs=p.parts.map(constructionSpec);
 $('#craft').innerHTML=`<h3>Tools & techniques</h3><p>${p.parts.some(x=>x.craftVersion)?'Rough construction keeps bark, irregular branches and fiber joints. Hewn wood needs earned practice and a carried tool.':'Earlier design: craftsmanship was not recorded. Shown as rough work; no skill credit has been invented.'}</p><p>${(bindingBudget(p).remainingMm/1000).toFixed(2)} m of cordage bindings remaining · ${(bindingBudget(p).atSiteMm/1000).toFixed(2)} m of cut cord at the site · ${bindingBudget(p).coilsNeeded} additional 4 m coils · ${specs.some(x=>x.tool)?'Hafted cutting edge required':'No precision tools required for these pieces'}</p>${designer?'<p>'+[...new Set(p.parts.filter(x=>!x.built).flatMap(x=>constructionBlockers(designer,x,p)))].map(esc).join(' · ')+'</p>':''}`;
 ...
 $('#parts').innerHTML=p.parts.map(x=>`<li><strong>${esc(x.id)}</strong> · ${esc(x.material)} × ${x.materialUnits}<br>${esc(constructionSpec(x).description)}${constructionSpec(x).binding?' · cordage '+((x.bindingLengthMm??(x.built||x.invested?(x.bindingUsed||0)*4000:bindingLength(x)))/1000).toFixed(2)+' m':''}<br><span class="muted">${x.built?'Assembled · '+Math.round(conditionOf(x)*100)+'% condition':x.invested?'Being assembled':'Materials needed'}${x.requires?.length?' · after '+x.requires.map(esc).join(', '):' · on the ground'}</span></li>`).join('');
...
function redraw(reset=false){$('#view-note').textContent=stage==='design'?'Finished design preview · every planned piece':'Recorded condition & progress · failed pieces lie at the site';viewer?.show(selected,$('#compare').checked?releasedStructureLook(selected):look,stage,reset);}
function saveLook(){
...
$('#program').ontoggle=async()=>{if(!$('#program').open)return;const p=selected,token=++codeRequest;$('#code-note').textContent=p.source==='example'?'Authored example program. This code creates a plan; it does not grant materials or completed construction.':'Each construction stage is shown in order. The programs are read-only here.';$('#code').textContent='Loading…';try{let code=p.code;if(!code&&p.source!=='example'){const response=await fetch(WORLD_BASE+'/design/'+encodeURIComponent(p.id),{signal:AbortSignal.timeout(10000),cache:'no-store'});if(!response.ok)throw Error('Source is unavailable while the world is disconnected.');const data=await response.json();code=constructionPrograms(data);}if(token===codeRequest)$('#code').textContent=code||'This older design uses a declarative list of parts.';}catch(e){if(token===codeRequest)$('#code').textContent=e.message;}};
