...
import {WORLD_ORIGIN,SNAPSHOT_KEY,catalogFromFrame,readCatalog,resourceRows,physicalFunctions} from './catalog.js';
...
async function refresh(){
 ...
 try{const response=await fetch(WORLD_ORIGIN+'/live/state',{signal:abort.signal,cache:'no-store'});const data=await response.json();if(!response.ok)throw Error(data.code==='hosting_limit'?'World paused by the hosting allowance.':data.error||'The world is unavailable.');catalog=catalogFromFrame(data);mode='snapshot';try{storage?.setItem(SNAPSHOT_KEY,JSON.stringify(catalog));}catch{}
