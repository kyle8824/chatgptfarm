...
export function householdWorld(w,a){
 ...
 const root=w.rootWorld||w,h=homeFor(root,a);if(!h)return root;
...
export function frontierGround(w,p){if(!w.frontier)return true;return !FRONTIER_OBSTACLES.some(r=>r.kind!=='terrace'&&pointIn(p,r,.35));}
export function terrainWaypoints(w,a,b){return w.frontier?FRONTIER_OBSTACLES.filter(r=>r.kind!=='terrace'&&(!a||Math.min(a.x,b.x)<r.maxX&&Math.max(a.x,b.x)>r.minX&&Math.min(a.y,b.y)<r.maxY&&Math.max(a.y,b.y)>r.minY)).flatMap(r=>[r.minX-1,r.maxX+1].flatMap(x=>[r.minY-1,r.maxY+1].map(y=>({x,y})))).filter(p=>p.x>0&&p.y>0&&p.x<FRONTIER_SIZE&&p.y<FRONTIER_SIZE):[];}
...
export function regionFor(p){return HOME_REGIONS.find(h=>Math.abs(p.x-h.x)<=55&&Math.abs(p.y-h.y)<=55)||null;}
