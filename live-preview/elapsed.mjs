...
export function prepare(seed){
 ...
 w.meta.persistentActions=1;w.meta.actionRulesVersion='elapsed-live-preview-1';w.minute=Number(w.minute||0);
...
export async function step(w,seconds,{mind=null,fallbackReason='no_provider',wallTime=Date.now()}={}){
 ...
 for(const a of agents){
  ...
  const t=a.task,from={...a.coordinates};let positionBefore=a.position;
  if(t?.phase==='travel'){
  ...
  a.runtimeMotion={from,to:{...a.coordinates},start:wallTime,end:wallTime+500,path:[from,{...a.coordinates}]};
 }
 w.minute+=minutes;
 advanceWood(w,w.day*24+w.hour+w.minute/60);
 if(w.minute>=60-1e-8){w.minute=Math.max(0,w.minute-60);finishHour(w,{needs:false,wood:false});updateSpectatorState(w);}
