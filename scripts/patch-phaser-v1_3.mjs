...
const must=(re,repl,label)=>{if(!re.test(s))throw new Error(`Missing patch anchor: ${label}`);s=s.replace(re,repl)};

must(/const TILE=24, UNITS=100, WORLD=TILE\*UNITS, POLL_MS=12000, MOVE_MS=9000;/,
`const TILE=24, UNITS=100, WORLD=TILE*UNITS, POLL_MS=12000, MOVE_MS=9000, SURFACE=640;`, 'surface constant');

must(/const pxToWorld=p=>\(\{x:p\.x\/TILE,y:UNITS-p\.y\/TILE\}\);/,
...
function fbm(x,y){return valueNoise(x,y,511)*.55+valueNoise(x*2.07,y*2.07,991)*.28+valueNoise(x*4.11,y*4.11,1559)*.17}`, 'noise helpers');

must(/for\(const n of \['004','005','006','007','008','009'\]\)this\.load\.image\(`tree-\$\{n\}`,`\$\{foliage\}\/foliagePack_\$\{n\}\.png`\);/,
`for(const n of ['004','005','006','007','008','009'])this.load.image(\`tree-\${n}\`,\`\${foliage}/foliagePack_\${n}.png\`);
  ...
  const carto=\`${ASSET_ROOT}/ui/cartography-pack/PNG/Default\`;this.load.image('camp-tent',\`${carto}/tent.png\`);this.load.image('campfire-art',\`${carto}/campfire.png\`);`, 'preload foliage/camp');

must(/this\.cameras\.main\.setZoom\(innerWidth<650\?\.98:1\.08\);/,
`this.cameras.main.setZoom(innerWidth<650?1.18:1.12);`, 'initial camera zoom');
...
` buildWorld(){
 ...
 drawOrganicSurface(){
  ...
  for(let y=0;y<SURFACE;y++){const wy=UNITS-y/SURFACE*UNITS;for(let x=0;x<SURFACE;x++){const wx=x/SURFACE*UNITS,p={x:wx,y:wy};let forest=0,wet=0;for(const t of terr){const q=terrainInfluence(t,p);if(t.type==='forest')forest=Math.max(forest,q);else if(t.type==='wetland')wet=Math.max(wet,q)}let col=[111,137,88];forest=smooth(clamp(forest*1.12,0,1));wet=smooth(clamp(wet*1.18,0,1));col=mixRGB(col,[58,86,57],forest*.82);col=mixRGB(col,[76,108,91],wet*.72);const cd=creekDistance(p),bank=smooth(clamp((4.9-cd)/3.5,0,1));col=mixRGB(col,[116,102,75],bank*.42);const n=fbm(wx*.075,wy*.075),fine=valueNoise(wx*.33,wy*.33,277);const shade=.87+n*.22+(fine-.5)*.055;const i=(y*SURFACE+x)*4;data[i]=clamp(col[0]*shade,0,255);data[i+1]=clamp(col[1]*shade,0,255);data[i+2]=clamp(col[2]*shade,0,255);data[i+3]=255}}
 ...
 drawBiomeUnderlays()`, 'organic surface/build');

must(/ drawBiomeUnderlays\(\)\{[\s\S]*?\n \}\n drawCreekDetails\(\)\{[\s\S]*?\n \}\n spawnGroundDetail\(\)\{[\s\S]*?\n \}\n spawnForest\(\)\{[\s\S]*?\n \}/,
` drawBiomeUnderlays(){const m=canonical.worldModel?.bounds?.metersPerUnit||2,R=seeded('biome-floor-v1_3');for(const t of canonical.worldModel?.terrain||[]){if(!['forest','wetland'].includes(t.type))continue;const p=worldToPx(t.center),r=(t.radiusM||20)/m*TILE,col=t.type==='forest'?0x263d2d:0x445f54;for(let i=0;i<(t.type==='forest'?18:12);i++){const a=R()*Math.PI*2,rr=Math.sqrt(R())*r*.72,x=p.x+Math.cos(a)*rr,y=p.y+Math.sin(a)*rr*.72,w=35+R()*95,h=18+R()*48;this.add.ellipse(x,y,w,h,col,.045+R()*.045).setDepth(-5)}}}
 ...
 }`, 'surface/creek/forest methods');

must(/const shelter=\(canonical\.worldModel\?\.objects\|\|\[\]\)\.find\(o=>o\.type==='branch_shelter'&&o\.state\?\.active\);if\(shelter\)\{[\s\S]*?\}const fire=\(canonical\.worldModel\?\.objects\|\|\[\]\)\.find\(o=>o\.type==='camp_fire'&&o\.state\?\.active\);if\(fire\)\{[\s\S]*?\}\n \}/,
`const shelter=(canonical.worldModel?.objects||[]).find(o=>o.type==='branch_shelter'&&o.state?.active);if(shelter){const p=worldToPx(shelter.position);this.add.ellipse(p.x+4,p.y+10,104,34,0x17251b,.18).setDepth(995+p.y);if(this.textures.exists('camp-tent')){const t=this.add.image(p.x,p.y+3,'camp-tent').setOrigin(.5,.82).setDisplaySize(92,92).setTint(0xb98258).setDepth(1000+p.y)}else this.add.triangle(p.x,p.y,0,46,40,0,80,46,0x74533b,.95).setDepth(1000+p.y)}const fire=(canonical.worldModel?.objects||[]).find(o=>o.type==='camp_fire'&&o.state?.active);if(fire){const p=worldToPx(fire.position),glow=this.add.circle(p.x,p.y+1,26,0xf4a64f,.085).setDepth(996+p.y);this.tweens.add({targets:glow,scale:1.18,alpha:.035,duration:760,yoyo:true,repeat:-1});const f=this.textures.exists('campfire-art')?this.add.image(p.x,p.y,'campfire-art').setDisplaySize(46,46):this.add.circle(p.x,p.y,8,0xef9540,.92);f.setDepth(1000+p.y+2);this.tweens.add({targets:f,scaleX:1.05,scaleY:.94,angle:.7,duration:460,yoyo:true,repeat:-1});for(let i=0;i<3;i++){const smoke=this.add.circle(p.x+(i-1)*3,p.y-12-i*4,4-i*.7,0xd7ded7,.14).setDepth(1002+p.y);this.tweens.add({targets:smoke,y:smoke.y-36,x:smoke.x+(i-1)*8,alpha:0,scale:1.8,duration:2100+i*420,repeat:-1,delay:i*390})}}
 }`, 'camp art');

must(/version:'phaser-v1\.2-living-world'/,`version:'phaser-v1.3-basin-slice'`,'version');
