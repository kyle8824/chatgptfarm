...
if "habitat-depth-v1" not in renderer:
    ...
    meadow=r''' drawMeadowMosaic(){const R=seeded('meadow-mosaic-v1_4_3');for(let i=0;i<46;i++){const p={x:3+R()*94,y:3+R()*94};if(terrainTypeAt(p)!==TERRAIN.MEADOW)continue;const q=worldToPx(p),w=90+R()*220,h=42+R()*120,col=R()<.5?0x617754:0x80916b;this.add.ellipse(q.x,q.y,w,h,col,.025+R()*.035).setRotation((R()-.5)*.7).setDepth(-8)}}
'''
    ...
    new_ground=r''' spawnGroundDetail(){const R=seeded('ground-life-v1_4_3');this.habitatDetail={meadow:0,edge:0,forest:0,wetland:0};for(let i=0;i<980;i++){const p={x:1+R()*98,y:1+R()*98},t=terrainTypeAt(p);if(t===TERRAIN.WATER||t===TERRAIN.BANK)continue;const q=worldToPx(p),nearForest=t===TERRAIN.MEADOW&&[[2,0],[-2,0],[0,2],[0,-2],[1.5,1.5],[-1.5,-1.5]].some(([dx,dy])=>terrainTypeAt({x:clamp(p.x+dx,0,100),y:clamp(p.y+dy,0,100)})===TERRAIN.FOREST);if(t===TERRAIN.FOREST){this.habitatDetail.forest++;if(R()<.57){const key='shrub-00'+(1+Math.floor(R()*3));if(this.textures.exists(key)){const b=this.add.image(q.x,q.y,key).setOrigin(.5,1).setScale(.12+.16*R()).setTint(R()<.5?0x62775b:0x52684f).setAlpha(.38+.32*R()).setDepth(900+q.y);if(R()<.1)this.tweens.add({targets:b,angle:(R()-.5)*1.2,duration:2600+R()*1500,yoyo:true,repeat:-1,ease:'Sine.InOut'});continue}}const litter=this.add.ellipse(q.x,q.y,5+R()*13,2+R()*5,R()<.55?0x4f5b43:0x675d45,.15+R()*.14).setDepth(40+q.y*.01);continue}if(t===TERRAIN.WETLAND){this.habitatDetail.wetland++;const tuft=this.add.image(q.x,q.y,'reeds').setOrigin(.5,1).setScale(.07+.09*R()).setAlpha(.3+.24*R()).setTint(R()<.5?0x82975c:0x6e8a5f).setDepth(920+q.y);if(R()<.18)this.tweens.add({targets:tuft,angle:(R()-.5)*2.2,duration:1800+R()*1600,yoyo:true,repeat:-1,ease:'Sine.InOut'});continue}this.habitatDetail.meadow++;if(nearForest&&R()<.48){this.habitatDetail.edge++;if(R()<.42){const key='shrub-00'+(1+Math.floor(R()*3));if(this.textures.exists(key)){this.add.image(q.x,q.y,key).setOrigin(.5,1).setScale(.08+.10*R()).setTint(0x70805f).setAlpha(.28+.22*R()).setDepth(880+q.y);continue}}}const g=this.add.graphics().setDepth(50+q.y*.01),stems=nearForest?3+Math.floor(R()*5):2+Math.floor(R()*4),base=nearForest?0x76865e:0x87946c;g.lineStyle(1,base,nearForest?.28:.18);for(let k=0;k<stems;k++){const ox=(R()-.5)*(nearForest?15:11),hh=(nearForest?7:4)+R()*(nearForest?15:10);g.lineBetween(q.x+ox,q.y,q.x+ox+(R()-.5)*4,q.y-hh)}if(R()<.08&&!nearForest)this.add.ellipse(q.x+(R()-.5)*10,q.y+(R()-.5)*5,10+R()*18,4+R()*8,0x657857,.08).setDepth(35)}}}
'''
