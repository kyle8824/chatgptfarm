...
const clayBank=state.worldModel?.objects?.find(x=>x.type==='clay_bank'&&x.state?.active!==false);if(clayBank?.position){const clayProjection=(await snap()).clayBankProjection;if(!clayProjection?.anchored)failures.push(`clay bank was not projected onto the creek shoreline: ${JSON.stringify(clayProjection)}`);if(!(clayProjection?.bankOffsetPx>=30&&clayProjection?.bankOffsetPx<=55))failures.push(`clay bank shoreline offset is invalid: ${JSON.stringify(clayProjection)}`);const focus=clayProjection?.worldAnchor||clayBank.position;await page.evaluate(({x,y})=>window.ChatGPTFarmPhaserDebug.focusWorldUnit(x,y,1.35),focus);await page.waitForTimeout(350);await page.screenshot({path:'phaser-qa/mobile-clay-bank-preview-2.png'});}
