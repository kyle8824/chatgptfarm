...
const clayBank=state.worldModel?.objects?.find(x=>x.type==='clay_bank'&&x.state?.active!==false);if(clayBank?.position){await page.evaluate(({x,y})=>window.ChatGPTFarmPhaserDebug.focusWorldUnit(x,y,1.35),clayBank.position);await page.waitForTimeout(350);await page.screenshot({path:'phaser-qa/mobile-clay-bank-preview.png'});}
