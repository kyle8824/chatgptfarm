...
import { updateSpectatorState } from '../engine/spectator.js';
...
await page.evaluate(()=>window.ChatGPTFarmPhaserDebug.reload());await page.waitForTimeout(800);s=await snap();if(!s.positions['W-RABBIT-001'])failures.push('same-page heartbeat lost rabbit');if(s.movingEntities<1)failures.push('heartbeat did not create visible movement');if(s.camera.mode!=='free')failures.push('heartbeat stole manual camera control');await page.screenshot({path:'phaser-qa/mobile-heartbeat.png'});
