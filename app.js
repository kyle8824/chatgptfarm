...
function normalize(w){
  if(!w.agents&&w.agent)w.agents=[w.agent,{id:'agent-ivo',name:'Ivo',position:'forest',needs:{hydration:74,hunger:68,energy:78,warmth:58},inventory:{},memories:[],beliefs:{},traits:{curiosity:.62,cooperation:.77,caution:.46},skills:{},experiments:{},currentAction:'Arriving at the basin'}];
  ...
  w.discoveries||=[];w.discovered||={};w.structures||={};
...
async function loadCanonical(){
  for(const url of [`${RAW_STATE}?t=${Date.now()}`,`./world/state.json?t=${Date.now()}`]){
    try{
      ...
      world=next;lastDiscoveryCount=world.discoveries.length;render(changed,newDiscovery);$('#saveState').textContent='● CANONICAL WORLD LIVE';return;
...
function renderFrontier(){
  const rows=[['●','Willow Basin','Creek, berries, fallen wood and stone field',true],['◒','Eastern clay bank','Sticky clay exposed by the creek',!!world.discovered.clayBank],['≋','Western reed marsh','Flexible reeds, marsh and animal traces',!!world.discovered.reedBed],['?','Beyond the basin','Unknown territory',false]];
...
function renderMemories(){
  const a=world.agents.find(x=>x.id===selectedMemoryAgent)||world.agents[0];$('#memoryTitle').textContent=`What ${a.name} remembers`;
...
function renderHistory(){
  ...
  $('#timeline').innerHTML=world.history.length?world.history.slice(0,55).map(e=>{const cls=e.type==='discovery'?'discovery-event':e.type==='social'||e.type==='culture'?'social-event':'';const titleCls=e.type==='learn'?'learn':e.type==='discovery'?'discovery-title':e.type==='social'||e.type==='culture'?'culture-title':'';return `<article class="event ${cls}"><time>D${e.day} · ${hourLabel(e.hour)}</time><div><b class="${titleCls}">${esc(e.title)}</b><p>${esc(e.detail)}</p></div></article>`}).join(''):'<div class="dna-empty">The canonical world has no history yet.</div>';
}

function renderDNA(){
  const dna=world.dna?.[0],el=$('#dnaInspector');if(!dna){el.innerHTML='<div class="dna-empty">Waiting for the first canonical Decision DNA trace.</div>';return}
...
function renderGuardian(){
  const analyses=world.agents.map(a=>{const dna=world.dna.find(d=>d.agent_id===a.id);if(!dna)return null;const pivotal=(dna.memory_counterfactuals||[]).filter(x=>x.action_changed),strongest=[...(dna.memory_counterfactuals||[])].sort((x,y)=>Math.abs(y.score_delta)-Math.abs(x.score_delta))[0];return{a,dna,pivotal,strongest}}).filter(Boolean);
  ...
  const bits=analyses.map(x=>x.pivotal.length?`${x.a.name}: ${x.pivotal[0].removed_memory} is pivotal — removing it changes ${x.dna.action} to ${x.pivotal[0].counterfactual_action}.`:x.strongest&&Math.abs(x.strongest.score_delta)>.01?`${x.a.name}: ${x.strongest.removed_memory} has the strongest measured memory effect (Δ ${x.strongest.score_delta.toFixed(2)}).`:`${x.a.name}: the latest choice is being driven mainly by current conditions.`);
  const d=world.discoveries?.[0];$('#guardianStatus').textContent=`${bits.join(' ')}${d?` Latest world-first: ${d.firstAgentName} discovered ${d.title.toLowerCase()} on Day ${d.day}.`:''}`;
}

function positionAgents(){world.agents.forEach((a,i)=>{const el=$(i===0?'#maraSprite':'#ivoSprite'),pos=POS[a.position]||POS.meadow;let left=parseFloat(pos[0]),bottom=parseFloat(pos[1]);if(i===1&&world.agents[0].position===a.position){left+=3;bottom+=1}el.style.left=`${left}%`;el.style.bottom=`${bottom}%`;el.dataset.action=a.currentAction||''})}

function render(changed=false,newDiscovery=false){
  if(!world)return;
  $('#weatherLabel').textContent=`${world.temperature}°F · ${world.weather[0].toUpperCase()+world.weather.slice(1)}`;$('#tickCount').textContent=world.meta?.tickNumber||0;$('#worldAge').textContent=`Day ${world.day}`;
  $('#shelter').classList.toggle('hidden',!world.structures?.shelter);$('#fire').classList.toggle('hidden',!world.structures?.fire);$('#cache').classList.toggle('hidden',!world.structures?.cache);$('#dryingRack').classList.toggle('hidden',!world.structures?.dryingRack);$('#clayBank').classList.toggle('hidden',!world.discovered?.clayBank);$('#reedBed').classList.toggle('hidden',!world.discovered?.reedBed);
  ...
  $('#worldCaption').textContent=world.history?.[0]?.detail||'Mara and Ivo are taking in Willow Basin.';
  ...
  if(changed){document.body.classList.add('world-updated');setTimeout(()=>document.body.classList.remove('world-updated'),1200)}if(newDiscovery){$('#discoveryPanel').classList.add('milestone-flash');setTimeout(()=>$('#discoveryPanel').classList.remove('milestone-flash'),1800)}
}
...
function updateProjectedClock(){
  ...
  const elapsed=Math.max(0,Date.now()-advanced),period=(world.meta?.heartbeatMinutes||5)*60000,hoursPerBeat=world.meta?.worldHoursPerHeartbeat||1;
  const worldMinutes=Math.min(hoursPerBeat*60-1,Math.floor((elapsed/period)*hoursPerBeat*60));
  const total=world.hour*60+worldMinutes,dayOffset=Math.floor(total/(24*60)),clock=total%(24*60),projectedHour=Math.floor(clock/60),projectedMinute=clock%60;
  $('#dayLabel').textContent=`Day ${world.day+dayOffset} · ${hourLabel(projectedHour,projectedMinute)}`;
  const remaining=Math.max(0,period-elapsed),mins=Math.floor(remaining/60000),secs=Math.floor((remaining%60000)/1000),pace=world.meta?.pacing||`${hoursPerBeat}× heartbeat`;
  $('#heartbeatLabel').textContent=elapsed>period?`Canonical heartbeat due now · ${pace}`:`Next canonical heartbeat ~${mins}:${String(secs).padStart(2,'0')} · ${pace}`;
}

function breathe(){if(!world)return;const t=Date.now()/1000;world.agents.forEach((a,i)=>{const el=$(i===0?'#maraSprite':'#ivoSprite'),base=POS[a.position]||POS.meadow;let x=parseFloat(base[0]),y=parseFloat(base[1]);if(i===1&&world.agents[0].position===a.position)x+=3;const amp=/Rest/i.test(a.currentAction)?0.15:0.7;x+=Math.sin(t*.55+i*2.1)*amp;y+=Math.cos(t*.42+i)*amp*.35;el.style.left=`${x}%`;el.style.bottom=`${y}%`})}
